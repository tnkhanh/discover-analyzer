...
let is_debug_mode () =
  !mode_debug || !mode_deep_debug

let is_interactive_mode () =
  !mode_interactive

let enable_mode_debug () =
  mode_debug := true

let disable_mode_debug () =
  mode_debug := false
...
let debug_core ?(ruler=`None) ?(prefix="") ?(indent=0) ~enable (printer: unit -> string) =
  if enable then
    let msg = printer () in
...
let hdebug ?(ruler=`None) ?(indent=0) ?(always=false) msg
      (pr: 'a -> string) (data: 'a) =
  let enable = not !no_debug && (!mode_debug || !mode_deep_debug || always) in
  let prefix = "\n" ^ msg in
...
let hdebugc ?(ruler=`None) ?(indent=0) ?(always=false) msg
      (pr: 'a -> string) (data: 'a) =
  let enable = not !no_debug && (!mode_debug || !mode_deep_debug || always) in
  let prefix = msg in
  debug_core ~ruler ~indent ~enable ~prefix (fun () -> pr data)
...
let hdebugln ?(ruler=`None) ?(indent=0) ?(always=false) msg
      (pr: 'a -> string) (data: 'a) =
  let enable = not !no_debug && (!mode_debug || !mode_deep_debug || always) in
  let prefix = "\n" ^ msg  in
  debug_core ~ruler ~indent ~enable ~prefix (fun () -> (pr data) ^ "\n")
...
let hddebug ?(ruler=`None) ?(indent=0) ?(always=false) msg
      (pr: 'a -> string) (data: 'a) =
  let enable = not !no_debug && (!mode_deep_debug || always) in
  let prefix = "\n" ^ msg in
  debug_core ~ruler ~indent ~enable ~prefix (fun () -> pr data)
...
let hddebugc ?(ruler=`None) ?(indent=0) ?(always=false) msg
      (pr: 'a -> string) (data: 'a) =
  let enable = not !no_debug && (!mode_deep_debug || always) in
  let prefix = msg in
  debug_core ~ruler ~indent ~enable ~prefix (fun () -> pr data)
...
(** high-order print a deep mode_debug message *)
let hddebugln ?(ruler=`None) ?(indent=0) ?(always=false) msg
      (pr: 'a -> string) (data: 'a) =
  let enable = not !no_debug && (!mode_deep_debug || always) in
  let prefix = "\n" ^ msg in
  debug_core ~ruler ~indent ~enable ~prefix (fun () -> (pr data) ^ "\n")
...
let display_choices msg (pr_choice: 'a -> string) (choices: 'a list) : string =
  let all_choices = choices |>
                    List.mapi ~f:(fun idx c ->
                      " [" ^ (pr_int (idx + 1)) ^ "]. " ^ (pr_choice c)) |>
                    String.concat ~sep:"\n" in
  let _ = print_endline (msg ^ "\n" ^ all_choices) in
  let range =
    let num_choices = List.length choices in
    if num_choices = 1 then "1"
    else "1" ^ ".." ^ (pr_int num_choices) in
  range
...
let rec ask_decision question (answer_choices: string list) : string =
  ...
  let is_valid_choice = List.exists ~f:(fun str ->
    if String.equal answer str then true
    else
      try
      ...
      with _ -> false) answer_choices in
  if not is_valid_choice then
    let _ = print_endline "\n>>> Choose again!" in
    ask_decision question answer_choices
  else answer

let nask_decision = fun _ _ -> ()
