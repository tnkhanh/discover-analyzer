...
let get_indent (msg: string) : int =
  ...
  if String.is_empty msg then 0
  else
    try
      for i = 0 to String.length msg do
        if (String.get msg i) != ' ' then
          raise (EInt i)
      done;
      String.length msg
    with EInt res -> res
...
let whitespace_of (length: int) : string =
  let rec mk_whitespace i =
    if (i <= 0) then ""
    else " " ^ (mk_whitespace (i - 1)) in
...
let pr_opt f x : string = match x with
  | None -> "None"
  | Some y -> "Some " ^ (f y)
...
let pr_indent ?(skipfirst=false) (indent: int) msg : string =
  ...
  msg |> String.split ~on:'\n' |>
  List.mapi ~f:(fun i s ->
    if i = 0 && skipfirst then s
    else sindent ^ s) |> String.concat ~sep:"\n"

(** high-order insert an indentation to each line of a message *)
let hpr_indent ?(skipfirst=false) (i: int) (f: 'a -> string) (v: 'a) : string =
...
let pr_prefix ~(prefix: string) msg : string =
  ...
  List.map ~f:(fun s -> prefix ^ s) |> String.concat ~sep:"\n"

let pr_list ?(sep=", ") ?(obrace="[") ?(cbrace="]")
      ?(indent="") ?(extra="") print items =
  let extra =
    if (String.equal obrace "") then extra
    else if (String.is_substring sep ~substring:"\n") then
      (whitespace_of (String.length obrace + 1)) ^ extra
    else extra in
...
let pr_args print items =
  pr_list ~sep:", " ~obrace:"" ~cbrace:"" print items
...
let beautiful_concat ?(column=80) ~(sep: string) (strs: string list) =
  let rec concat strs current_line res =
    match strs with
    | [] ->
      if String.is_empty current_line then res
      else if String.is_empty res then current_line
      else (String.strip res) ^ "\n" ^ current_line
    | str::nstrs ->
      let new_line =
        if List.is_empty nstrs then current_line ^ str
        else current_line ^ str ^ sep in
      if String.length new_line <= column then
        concat nstrs new_line res
      else
        let res =
          if String.is_empty res then current_line
          else (String.strip res) ^ "\n" ^ current_line in
        if List.is_empty nstrs then concat nstrs str res
