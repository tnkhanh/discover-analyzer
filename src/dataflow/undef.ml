...
module UndefDomain = struct
  ...
  let lequal_undef (ud1 : undef) (ud2 : undef) : bool =
    ...
    if num_pointers1 < num_pointers2
    then true
    else if num_pointers1 > num_pointers2
    then false
...
struct
  ...
  let analyze_instr ?(widen = false) (penv : prog_env) fenv instr (input : t)
    ...
    | LO.Store ->
      ...
      if UD.is_undef_value input src
      then UD.insert_undef_pointer input dst
      else UD.remove_undef_pointer input dst
    | LO.Load ->
      let src = src_of_instr_load instr in
      let dst = dst_of_instr_load instr in
      if UD.is_undef_pointer input src
      then UD.insert_undef_value input dst
    ...
    | LO.PHI ->
      ...
      if List.exists ~f:(UD.is_undef_value input) srcs
      then (
