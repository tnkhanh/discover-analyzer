...
module IntervalDomain = struct
  ...
  let mult_range (a: range) (b: range) =
    ...
    let b1, i1 = mult_bound a.range_lb b.range_lb,
                 a.range_lb_inclusive && b.range_lb_inclusive in
    let b2, i2 = mult_bound a.range_lb b.range_ub,
                 a.range_lb_inclusive && b.range_ub_inclusive in
    let b3, i3 = mult_bound a.range_ub b.range_lb,
                 a.range_ub_inclusive && b.range_lb_inclusive in
    let b4, i4 = mult_bound a.range_ub b.range_ub,
  ...
  let union_range (a: range) (b: range) : range =
    let lb, li =
      ...
      if cmp < 0 then (a.range_lb, a.range_lb_inclusive)
      else if cmp > 0 then (b.range_lb, b.range_lb_inclusive)
      else (a.range_lb, a.range_lb_inclusive && b.range_lb_inclusive) in
    let ub, ui =
      let cmp = compare_bound a.range_ub b.range_ub in
      if cmp > 0 then (a.range_ub, a.range_ub_inclusive)
      else if cmp < 0 then (b.range_ub, b.range_ub_inclusive)
  ...
  let widen_range (a: range) (b: range) : range =
    let lb, li =
      if compare_bound a.range_lb b.range_lb <= 0 then
        (a.range_lb, a.range_lb_inclusive)
      else (NInf, false) in
    let ub, ui =
      if compare_bound a.range_ub b.range_ub >= 0 then
        (a.range_ub, a.range_ub_inclusive)
      else (PInf, false) in
    mk_range lb ub ~li ~ui
  ...
  let intersect_interval (a: interval) (b: interval) : interval =
    ...
    | Range ra, Range rb ->
      let lb, li =
        ...
        if cmp > 0 then (ra.range_lb, ra.range_lb_inclusive)
        else if cmp < 0 then (rb.range_lb, rb.range_lb_inclusive)
        else (ra.range_lb, ra.range_lb_inclusive && rb.range_lb_inclusive) in
      let ub, ui =
        let cmp = compare_bound ra.range_ub rb.range_ub in
        if cmp < 0 then (ra.range_ub, ra.range_ub_inclusive)
        else if cmp > 0 then (rb.range_ub, rb.range_ub_inclusive)
...
struct
    ...
    let refine_interval (a: interval) (b: interval) : interval =
      ...
      | Range ra, Range rb ->
        let lb, li =
          let cmp = compare_bound ra.range_lb rb.range_lb in
          if cmp > 0 then (ra.range_lb, ra.range_lb_inclusive)
          else if cmp < 0 then (ra.range_lb, ra.range_lb_inclusive)
          else (ra.range_lb, ra.range_lb_inclusive && rb.range_lb_inclusive) in
        let ub, ui =
          let cmp = compare_bound ra.range_ub rb.range_ub in
          if cmp < 0 then (ra.range_ub, ra.range_ub_inclusive)
          else if cmp > 0 then (ra.range_ub, ra.range_ub_inclusive)
          else (ra.range_ub, ra.range_ub_inclusive && rb.range_ub_inclusive) in
        if compare_bound lb ub > 0 then Bottom
        else Range (mk_range lb ub ~li ~ui) in
    let pcond_data = extract_data_from_predicate ~widen p d in
    MP.merge
      ~f:(fun ~key:e i -> match i with
  ...
  let refine_widening func instr widen : bool =
    ...
    let has_int_cmp_relation = List.exists
                                 ~f:(fun u -> match LL.classify_value u with
  ...
  let check_range_lower_bound fenv instr (v: llvalue) (lb: int64) : bool =
    ...
    | Some data ->
      ...
      | Range r ->
        ...
        | NInf -> false
        | Int64 i ->
          let vlb = (if r.range_lb_inclusive then i
  ...
  let check_range_upper_bound fenv instr (v: llvalue) (ub: int64) : bool =
    ...
    | Some data ->
      ...
      | Range r ->
        ...
        | NInf -> true
        | Int64 i ->
          let vub = (if r.range_ub_inclusive then i
                     else Int64.(-) i Int64.one) in
          Int64.(<=) vub ub
        | BInt i ->
          let vub = (if r.range_ub_inclusive then i
                     else BInt.sub_big_int i BInt.one) in
    ...
    | AS.Assert, AS.RangeFull (v::lb::ub::_) ->
      (match LL.int64_of_const lb, LL.int64_of_const ub with
       ...
       | Some lb, Some ub ->
         Some (List.exists
                 ~f:(fun fe ->
                      check_range_full fe instr v lb ub)
                 fenvs))
    | _ -> None
  ...
  let check_assertions (penv: prog_env) func : int =
    ...
    let _ = List.iter
              ~f:(fun ast -> match check_assertion fenvs ast with
                   | Some res ->
                     let _ = incr num_checked_assertions in
                     let _ = incr (if res then num_valid_asserts
                                   else num_invalid_asserts) in
