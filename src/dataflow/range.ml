...
struct
  ...
  let subst_data
    ...
    MP.fold
      ~f:(fun ~key:e ~data:i acc ->
        ...
        | `Ok res -> res
        | `Duplicate ->
          let _ = warningp "Range.subst_data: duplicate expr: " pr_expr ne in
          acc)
      ~init:MP.empty d
  ;;
  ...
  let extract_data_from_predicate ~(widen : bool) (p : predicate) (d : t) : t =
    ...
    | PIcmp (cmp, lhs, rhs) ->
      ...
      then (
        let b =
          ...
          | Some b -> b
          | None -> errorp "extract_const_bound_lhs: not found:" pr_expr lhs
        in
        match cmp with
      ...
      then (
        let b =
          ...
          | Some b -> b
          | None -> errorp "extract_const_bound_rhs: not found:" pr_expr rhs
        in
        match cmp with
    ...
    | PNeg _ | PConj _ | PDisj _ ->
      errorp "extract_data_from_predicate: need to handle: " pr_predicate p
  ;;
  ...
  let prepare_entry_func_input (penv : prog_env) func (input : t) : t =
    ...
    List.fold_left
      ~f:(fun acc param ->
        let _ = debugp "param: " pr_param param in
  ...
  let analyze_instr
    ...
    | LO.Call | LO.Invoke ->
      ...
      let _ = printp "Function name: " pr_str fname in
