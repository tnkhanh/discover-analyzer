...
module PointerGraph = struct
  ...
  let src_of_path (path : path) : vertex =
    ...
    | [] -> errorh "src_of_path: empty path: " pr_path path
    | e :: _ -> E.src e
  ;;

  let dst_of_path (path : path) : vertex =
    match List.last path.path_edges with
    | None -> errorh "dst_of_path: empty path: " pr_path path
    | Some e -> E.dst e
  ;;
  ...
  let is_valid_path_trace prog (g : t) (path : path) : bool =
    ...
    let check_read_write_flow () =
      ...
      List.for_all
        ~f:(fun (dexp, trs) ->
          (* let _ = debugh "check read_write of dexp: " pr_expr dexp in *)
...
module PointerDomain = struct
  ...
  let simplify_labels_for_alias_check (labels : PE.label list) : PE.label list =
    ...
    (* let _ = debugh ~indent:8 "After simplifying labels: " PG.pr_labels labels in *)
    let labels = accumulate_gep_indices labels [] in
    (* let _ = debugh ~indent:8 "After accumulating gep indices: " PG.pr_labels labels in *)
    let labels = collapse_gep_indices labels in
    (* let _ = debugh "After collapsing gep indices: " PG.pr_labels labels in *)
  ...
  let check_path_balance_direction path (p : precision) : alias_path_type =
              ...
              then check_balance nlabels nvisited
                ...
                | Int64 i1 :: nridxs1, Int64 i2 :: nridxs2 ->
                  ...
                  then (
                    ...
                    (* let _ = debugh "New Label: " PG.pr_label lbl in *)
    ...
    (* let _ = debugh "Path: " pr_path path in *)
    let labels = path.path_flattened_labels in
    (* let _ = debugh "Original labels: " PG.pr_labels labels in *)
    let labels = simplify_labels_for_alias_check labels in
    (* let _ = debugh "After simplifying labels: " PG.pr_labels labels in *)
  ...
  let check_alias_all_paths_bfs
    ...
    let rec search_bfs dst (queue : path list) : bool * path option =
      (* let _ = debugh ~always:true "Path queue BEFORE sorting: " pr_paths queue in *)
      let queue =
        List.sorti
          ~compare:(fun p1 p2 -> compare_path shortest_distances p1 p2 v2)
          queue in
      (* let _ = debugh ~always:true "AFTER sorting: " pr_paths queue in *)
      match queue with
      | [] -> false, None
      | path :: nqueue ->
  ...
  let find_deref_paths (g : pgraph) (src : expr) : path list =
    (* let _ = printh "Find deref paths of " pr_expr src in *)
  ...
  let find_funcs_of_pointer (prog : program) (g : pgraph) (ptr : value) : funcs
    ...
    let _ = debugh "Finding functions pointed by: " pr_value ptr in
    (* let _ = debugh "  - Input: " pr_pgraph g in *)
    let vptr = expr_of_llvalue ptr in
    let vfuncs = get_func_vertices_in_pgraph g in
    let _ = debugh "Functions in pgraph: " pr_exprs vfuncs in
    let funcs =
      List.fold_left
        ~f:(fun acc v ->
    ...
    let _ = debugh "  Output function pointers: " func_names funcs in
...
struct
  ...
  let remove_deref_of_vertex ?(trace_skip = []) (input : t) (v : expr) : unit =
    (* let _ = debugh ~indent:4 "Remove deref vertex: " pr_expr v in *)
                ...
                ~f:(fun e2 ->
                  ...
                  then (
                    ...
                    | Some nlbl ->
                      ...
                      (* let _ = debugh "  remove edge: " PG.pr_edge e1 in *)
                      let _ =
                        if PG.is_alias_edge e2 then PG.remove_edge_e g e2 in
                      (* let _ = debugh "  remove edge: " PG.pr_edge e2 in *)
  ...
  let eliminate_vertex_by_join_edges (g : pgraph) ?(kept_vs = None) (v : expr)
    ...
    let connected_kept_vs =
      ...
      | None -> []
      | Some kept_vs ->
        (* let _ = debugh "kept_vs: " pr_exprs kept_vs in *)
  ...
  let join_edges_of_irrelevant_vars penv func (g : pgraph) : unit =
    ...
    (* let _ = debugh "Before joining edges: " pr_data g in *)
    let _ = remove_must_alias_vertices penv func g in
    (* let _ = debugh "After remove must-alias: " pr_data g in *)
    (* second, remove alias by joining edges *)
    let _ = eliminate_alias_edges_by_join_edges penv func g in
    (* let _ = debugh "After join alias edges: " pr_data g in *)
    ...
    let removable_vs, kept_vs =
      PG.fold_vertex
        ...
        g ([], []) in
    (* let _ = debugh "removable_vs: " pr_exprs removable_vs in *)
    ...
    let _ =
      List.iter
        ~f:(fun v ->
          ...
          let _ = debugh "Remove vertex: " pr_expr ev in
  ...
  let is_global_used_only_in_init_sparse_functions penv (g : global) : bool =
    ...
    (* let _ = printh "check global: " pr_global g in *)
    ...
    List.for_all
      ~f:(fun u ->
        ...
        | LV.Instruction LO.Store | LV.Instruction LO.GetElementPtr ->
          ...
          (* let _ = printh "   func: " func_name func in *)
          is_init_func func
        | _ -> false)
      users
  ;;
  ...
  let refine_candidate_sparse_globals penv : bool =
    ...
    let is_global_used_only_as_dst_of_instr_store g =
      let vg = llvalue_of_global g in
      (* let _ = printh "global: " pr_global g in *)
      for_all_use
        ~f:(fun u ->
          let vu = LL.user u in
          if is_sparse_llvalue penv vu
          then (
            (* let _ = printh "  user: " pr_value_detail vu in *)
            match LL.classify_value vu with
  ...
  let refine_candidate_sparse_instrs penv : bool =
    ...
    let rec refine_func f : unit =
      (* let _ = debugh "Refining sparse function: " func_name f in *)
  ...
  let prepare_callee_input penv instr callee args (input : t) : t =
    ...
    let _ = debugh ~indent:4 "Prepare callee's input: " func_name callee in
    let _ = debugh ~indent:4 " - Initial input: " pr_data input in
    let params =
      let params = formal_params_of_func callee in
      List.map
        ~f:(fun p -> get_bitcast_alias_of_llvalue (llvalue_of_param p))
        params in
    let _ = debugh ~indent:4 " - args: " pr_exprs args in
    let _ = debugh ~indent:4 " - params: " pr_exprs params in
  ...
  let analyze_instr ?(widen = false) penv fenv (instr : instr) (input : t) : t =
    ...
    | LO.Store when is_llvalue_contain_pointer (operand instr 0) ->
      ...
      then input
        ...
        (* let _ = printh "handling store: " pr_instr instr in *)
        (* let _ = printh "  graph size: " pr_graph_size input in *)
        ...
        let _ =
          ...
          then (
            ...
            (* let _ = printh "finish finding all aliases of " pr_expr dexp in *)
    ...
    | LO.Call | LO.Invoke ->
      ...
      then (
        let _ = debugh "==> CALL TO DELETE: " func_name callee in
      ...
      then (
        ...
        let _ = debugh "Src type: " pr_type styp in
        let elem_typ = LL.element_type styp in
        let _ = debugh "Elem type: " pr_type elem_typ in
        let _ =
        ...
        (* let _ = debugh "   handle Memcpy: " pr_expr sexp in
         * let _ = debugh "   New Edges: " (pr_items PG.pr_edge) edges in *)
        output)
      ...
      then (
        ...
        let _ = debugh "pointer: callee: " func_name callee in
        let ptr = llvalue_of_func callee in
        let _ = debugh "Find function pointer in graph: " pr_pgraph output in
        let funcs =
