...
functor
  ...
  struct
    ...
    let get_sparse_preceding_blocks ?(cache = true) penv blk : block list =
      ...
      then
        Hashtbl.find_or_compute
          penv.penv_sparse_precedings_block
          ~key:blk
          ~f:(fun () -> compute blk)
      else compute blk
    ;;
    ...
    let get_sparse_succeeding_blocks ?(cache = true) penv blk : block list =
      ...
      then
        Hashtbl.find_or_compute
          penv.penv_sparse_succeedings_block
          ~key:blk
          ~f:(fun () -> compute blk)
      else compute blk
    ;;
    ...
    let get_sparse_reachable_blocks penv (blk : block) : blocks =
      ...
      Hashtbl.find_or_compute
        penv.penv_sparse_reachable_blocks
        ~key:blk
        ~f:compute
    ;;
              ...
              | Some output ->
                ...
                let output =
                  ...
                  | Some pred ->
                    let _ =
                      hdebug
                        "Preceding block's current output: "
                        T.sprint_data
                        output in
                    let new_output =
                    ...
                    let _ =
                      hdebug
                        "Preceding block's refined output: "
                        T.sprint_data
                        new_output in
                    new_output in
          ...
          |> List.map ~f:(fun args ->
                 ...
                 let _ =
                   if get_func_analyzed_times penv callee == 0
                      || not
                           (equal_args_data_vs_analyzed_inputs
                              penv
                              callee
                              args
                              callee_input)
                   then (
                     ...
                     let _ =
                       hdebug
                         ~indent:4
                         "Mark working block to re-analyze: "
                         pr_working_block
                         wb in
                     let working_blks =
                       List.insert_dedup
                         fenv.fenv_working_blocks
                         wb
                 ...
                 | Some fsum ->
                   ...
                   let output, exns =
                     compute_call_output_exns_from_summary
                       penv
                       instr
                       callee
                       args
    ...
    let analyze_function penv (wf : working_func)
      ...
      let _ =
        hdebug
          ~always:true
          " - Re-analyzing from working blocks: "
          pr_working_blocks
