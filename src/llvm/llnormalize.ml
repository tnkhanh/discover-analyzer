...
let rename_vars_and_params (modul: LL.llmodule) : unit =
  ...
  let finstr = Some (fun instr ->
    ...
    | LO.Br _ | LO.IndirectBr | LO.Resume
    | LO.Store | LO.Ret | LO.Unreachable -> ()
    | LO.Call when is_type_void (type_of_instr instr) -> ()
    | LO.Invoke when is_type_void (type_of_instr instr) -> ()
    | _ ->
      if not !llvm_orig_source_name || is_llvalue_empty_name vinstr then
  ...
  let fglobal = Some (fun global ->
    ...
    if not !llvm_orig_source_name || is_llvalue_empty_name vglobal then
...
let check_normalization (modul: LL.llmodule) : unit =
  ...
  let fblock = Some (fun blk ->
    let _ = match last_instr_of_block blk with
      | None -> ()
      | Some instr -> match instr_opcode instr with
        | LO.Ret | LO.Br | LO.IndirectBr | LO.Switch
        | LO.Call | LO.Invoke
        | LO.Resume | LO.Unreachable -> ()
        | _ -> error ("The last instruction of block '" ^ (block_name blk) ^
                      "' is: " ^ (pr_instr instr) ^ "\n" ^
