...
let get_loop_blocks loop : block list =
  loop.loop_head::loop.loop_body

let find_loop_head (prog: program) (scb: LG.scblocks) : block option =
  (* let _ = hprint "Find_loop_head in: " LG.pr_scblocks scb in *)
  let head_blks = List.fold_left ~f:(fun acc blk ->
    let pblks = get_preceding_blocks prog blk in
    let has_outside_preceding = List.exists ~f:(fun pb ->
      not (List.mem scb pb.pblk_block ~equal:(==))) pblks in
    if has_outside_preceding then acc @ [blk]
    else acc
  ) ~init:[] scb in
  ...
  | [blk] -> Some blk
  | _ ->
    let msg = "Too many loop heads: " ^ (block_names head_blks) ^ "\n" ^
...
let find_loop_exit (prog: program) (scb: LG.scblocks) : blocks =
  ...
  let exit_blocks = List.fold_left ~f:(fun acc1 blk ->
    let sblks = get_succeeding_blocks prog blk in
    List.fold_left ~f:(fun acc2 sblk ->
      let blk_succ = sblk.sblk_block in
      if SP.mem scblocks blk_succ then acc2
      else SP.add acc2 blk_succ
    ) ~init:acc1 sblks
  ) ~init:SP.empty scb in
  SP.to_list exit_blocks

let update_loop_info (prog: program) loop : loop =
  let exit_reachables = List.fold_left ~f:(fun acc blk ->
...
let find_loop_in_blocks (prog: program) (blks: blocks) : loops =
  ...
  let loops = List.fold_left ~f:(fun acc scb ->
    if List.length scb <= 1 then acc
    else match find_loop_head prog scb with
      | None -> acc
      | Some head ->
...
let get_loops_of_func (prog: program) (func: func) : loops =
  Hashtbl.find_or_compute prog.prog_func_loops ~key:func
    ~f:(fun () -> find_loop_in_func prog func)

let find_innermost_loop_of_block (prog: program) blk : loop option =
  let find_loop v =
    let rec find_innermost_loop loop blk : loop =
      let loop_opt = List.find ~f:(fun lp ->
        List.mem ~equal:equal_block (get_loop_blocks lp) blk) loop.loop_inners in
      match loop_opt with
      | None -> loop
    ...
    let loops = get_loops_of_func prog func in
      let loop_opt = List.find ~f:(fun lp ->
        List.mem ~equal:equal_block (get_loop_blocks lp) blk) loops in
      match loop_opt with
      | None -> None
      | Some loop -> Some (find_innermost_loop loop blk) in
  Hashtbl.find_or_compute prog.prog_block_innermost_loop ~key:blk
    ~f:(fun () -> find_loop blk)


let find_innermost_loop_of_llvalue (prog: program) (v: llvalue) : loop option =
  let find_loop v =
    if is_llvalue_instr v then
      let blk = block_of_instr (mk_instr v) in
      find_innermost_loop_of_block prog blk
    else None in
  Hashtbl.find_or_compute prog.prog_llvalue_innermost_loop ~key:v
    ~f:(fun () -> find_loop v)
...
let is_loop_updated_instr prog instr : bool =
  let check_instr instr =
    ...
    let loops = get_loops_of_func prog func in
    List.exists ~f:(fun lp ->
      List.exists ~f:(equal_block blk) (lp.loop_head::lp.loop_body)
    ) loops in
  Hashtbl.find_or_compute prog.prog_loop_updated_instr ~key:instr
...
let is_loop_head_instr prog instr : bool =
  ...
  Hashtbl.find_or_compute prog.prog_loop_head_instr ~key:instr
    ~f:(fun () -> check_instr instr)

let is_loop_updated_llvalue prog v : bool =
  if is_llvalue_instr v then
    is_loop_updated_instr prog (mk_instr v)
  else false

let is_loop_head_llvalue prog (v: llvalue) : bool =
  if is_llvalue_instr v then
    is_loop_head_instr prog (mk_instr v)
  else false
