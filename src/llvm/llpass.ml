  ...
  let finstr = Some (fun instr ->
    ...
    | LO.Call | LO.Invoke ->
      ...
      if is_llvalue_function vcallee then (
        let callers = List.insert_dedup (get_func_callers prog callee)
                        caller ~equal in
        let _ = Hashtbl.set prog.prog_func_callers ~key:callee ~data:callers in
        let callees = List.insert_dedup (get_func_callees prog caller)
                        callee ~equal in
        Hashtbl.set prog.prog_func_callees ~key:caller ~data:callees)
      else
        let pcallees = get_func_ptr_callees prog caller in
        let pcallees = List.insert_dedup pcallees vcallee
                            ~equal:equal_llvalue in
        Hashtbl.set prog.prog_func_ptr_callees ~key:caller ~data:pcallees
    | _ -> ()) in
  deep_iter_program ~finstr prog

let construct_func_call_graph (prog: program) : unit =
  Hashtbl.iteri ~f:(fun ~key:func ~data:callees ->
...
let compute_funcs_in_pointers (prog: program) : unit =
  ...
  let finstr = Some (fun instr ->
    let func = match instr_opcode instr with
      | LO.BitCast ->
        let v = src_of_instr_bitcast instr in
        if is_llvalue_function v then Some (mk_func v)
        else None
      | LO.Store ->
    ...
    | Some f ->
      ...
      let curr_funcs = match Hashtbl.find prog.prog_funcs_in_pointers ftyp with
        | None -> []
        | Some fs -> fs in
...
let compute_func_used_globals prog : unit =
  ...
  let init_globals_of_all_funcs () =
    let _ = List.iter ~f:(fun func ->
      ...
      Hashtbl.set tbl_used_globals ~key:func ~data:gs) prog.prog_init_funcs in
