...
let pr_scblocks (scb: scblocks) : string =
  pr_list block_name scb
...
let build_reachability_graph (prog: program) : IG.t =
  ...
  let _ = if IG.is_empty rg then
      ...
      let finstr = Some (fun instr ->
        ...
        let next_instrs = match instr_succ instr with
          | Some instr' -> [instr']
          | None ->
            match instr_opcode instr with
            | LO.Br | LO.IndirectBr ->
              let next_blocks = match branch_of_instr_br instr with
                | None -> []
                | Some (`Conditional (_, b1, b2)) -> [b1; b2]
                | Some (`Unconditional b) -> [b] in
              List.fold_left ~f:(fun acc b ->
                match first_instr_of_block b with
                | None -> acc
                | Some i' -> acc @ [i']) ~init:[] next_blocks
            | _ -> [] in
        let _ = List.iter ~f:(fun instr' ->
        ...
        | _ -> ()) in
      deep_iter_program ~ffunc ~finstr prog in
  rg
