...
module Instr = struct
  ...
  let dst_of_instr_alloca (i : instr) : value =
    ...
    | _ -> errorp "dst_of_instr: not an instr Alloca: " pr_instr i
  ;;
  ...
  let src_of_instr_store (i : instr) : value =
    ...
    | _ -> errorp "src_of_instr: not an instr Store: " pr_instr i
  ;;

  let dst_of_instr_store (i : instr) : value =
    match instr_opcode i with
    | LO.Store -> operand i 1
    | _ -> errorp "dst_of_instr: not an instr Store: " pr_instr i
  ;;
  ...
  let src_of_instr_load (i : instr) : value =
    match instr_opcode i with
    | LO.Load -> operand i 0
    | _ -> errorp "src_of_instr: not an instr Load: " pr_instr i
  ;;

  let dst_of_instr_load (i : instr) : value =
    match instr_opcode i with
    | LO.Load -> llvalue_of_instr i
    | _ -> errorp "dst_of_instr: not an instr Load: " pr_instr i
  ;;
  ...
  let src_of_instr_insertvalue (i : instr) : value =
    ...
    | _ -> errorp "src_of_instr: not an instr InsertValue: " pr_instr i
  ;;

  let dst_of_instr_insertvalue (i : instr) : value =
    match instr_opcode i with
    | LO.InsertValue -> operand i 0
    | _ -> errorp "dst_of_instr: not an instr InsertValue: " pr_instr i
  ;;
  ...
  let src_of_instr_extractvalue (i : instr) : value =
    ...
    | _ -> errorp "src_of_instr: not an instr ExtractValue: " pr_instr i
  ;;

  let dst_of_instr_extractvalue (i : instr) : value =
    match instr_opcode i with
    | LO.ExtractValue -> llvalue_of_instr i
    | _ -> errorp "dst_of_instr: not an instr ExtractValue: " pr_instr i
  ;;
  ...
  let src_of_instr_gep (i : instr) : value =
    ...
    | _ -> errorp "src_of_instr: not an instr GEP: " pr_instr i
  ;;

  let dst_of_instr_gep (i : instr) : value =
    match instr_opcode i with
    | LO.GetElementPtr -> llvalue_of_instr i
    | _ -> errorp "dst_of_instr: not an instr GEP: " pr_instr i
  ;;
  ...
  let src_of_instr_gep_extract_value (i : instr) : value =
    ...
    | LO.GetElementPtr -> operand i 0
    | LO.ExtractValue -> operand i 0
    | _ -> errorp "src_of_instr: not an instr GEP/ExtractValue: " pr_instr i
  ;;
  ...
  let dst_of_instr_gep_extract_value (i : instr) : value =
    ...
    | LO.GetElementPtr -> llvalue_of_instr i
    | LO.ExtractValue -> llvalue_of_instr i
    | _ -> errorp "dst_of_instr: not an instr GEP/ExtractValue: " pr_instr i
  ;;
  ...
  let src_of_instr_bitcast (i : instr) : value =
    ...
    | _ -> errorp "src_of_instr: not an instr BitCast: " pr_instr i
  ;;

  let dst_of_instr_bitcast (i : instr) : value =
    match instr_opcode i with
    | LO.BitCast -> llvalue_of_instr i
    | _ -> errorp "dst_of_instr: not an instr BitCast: " pr_instr i
  ;;
  ...
  let src_of_instr_return (i : instr) : value =
    match instr_opcode i with
    | LO.Ret -> operand i 0
    | _ -> errorp "src_of_instr: not an instr FuncRes: " pr_instr i
  ;;
  ...
  let src_of_instr_sext (i : instr) : value =
    match instr_opcode i with
    | LO.SExt -> operand i 0
    | _ -> errorp "src_of_instr: not an instr SExt: " pr_instr i
  ;;

  let dst_of_instr_sext (i : instr) : value =
    match instr_opcode i with
    | LO.SExt -> llvalue_of_instr i
    | _ -> errorp "dst_of_instr: not an instr SExt: " pr_instr i
  ;;
  ...
  let src_of_instr_zext (i : instr) : value =
    match instr_opcode i with
    | LO.ZExt -> operand i 0
    | _ -> errorp "src_of_instr: not an instr ZExt: " pr_instr i
  ;;

  let dst_of_instr_zext (i : instr) : value =
    match instr_opcode i with
    | LO.ZExt -> llvalue_of_instr i
    | _ -> errorp "dst_of_instr: not an instr ZExt: " pr_instr i
  ;;
  ...
  let num_args_of_instr_call (i : instr) : int =
    ...
    | _ -> errorp "num_args_of: not an instr Call: " pr_instr i
  ;;
  ...
  let callee_of_instr_call (i : instr) : func =
    ...
    | _ -> errorp "callee_of: not an instr Call: " pr_instr i
  ;;
  ...
  let arg_of_instr_call (i : instr) (idx : int) : value =
    ...
    | LO.Call ->
      ...
      then operand i idx
      else errorp "arg_of_instr_call: idx out of bound" pr_int idx
    | _ -> errorp "arg_of: not an instr Call: " pr_instr i
  ;;
  ...
  let args_of_instr_call (i : instr) : values =
    ...
    | LO.Call ->
      ...
      !args
    | _ -> errorp "operand_args: not an instr Call: " pr_instr i
  ;;
  ...
  let num_args_of_instr_callbr (i : instr) : int =
    ...
    | _ -> errorp "num_args_of: not an instr CallBr: " pr_instr i
  ;;
  ...
  let args_of_instr_callbr (i : instr) : values =
    ...
    | LO.CallBr ->
      ...
      !args
    | _ -> errorp "operand_args: not an instr CallBr: " pr_instr i
  ;;
  ...
  let callee_of_instr_callbr (i : instr) : func =
    ...
    | LO.CallBr ->
      ...
      mk_func (operand i num_args)
    | _ -> errorp "callee_of: not an instr CallBr: " pr_instr i
  ;;
  ...
  let num_args_of_instr_invoke (i : instr) : int =
    ...
    | _ -> errorp "num_args_of: not an instr Invoke: " pr_instr i
  ;;
  ...
  let arg_of_instr_invoke (i : instr) (idx : int) : value =
    ...
    | LO.Invoke ->
      if idx < num_args_of_instr_call i
      then operand i idx
      else errorp "arg_of_instr_invoke: idx out of bound" pr_int idx
    | _ -> errorp "arg_of: not an instr Invoke: " pr_instr i
  ;;
  ...
  let args_of_instr_invoke (i : instr) : values =
    ...
    | LO.Invoke ->
      ...
      !args
    | _ -> errorp "operand_args: not an instr Invoke: " pr_instr i
  ;;
  ...
  let num_args_of_instr_func_app (i : instr) : int =
    ...
    | _ ->
      errorp "num_args_of_instr_func_app: not a callable instr: " pr_instr i
  ;;
  ...
  let callee_of_instr_func_call (i : instr) : func =
    ...
    | _ ->
      errorp "callee_of_instr_func_call: not a callable instr: " pr_instr i
  ;;
  ...
  let arg_of_instr_func_app (i : instr) (idx : int) : value =
    ...
    | _ -> errorp "arg_of_instr_func_app: not a callable instr: " pr_instr i
  ;;
  ...
  let args_of_instr_func_app (i : instr) : values =
    ...
    | _ -> errorp "args_of_instr_func_app: not a callable instr: " pr_instr i
  ;;
  ...
  let get_origin_src_of_memcpy (i : instr) : value =
    ...
    else errorp "get_origin_src_of_memcpy: not a memcopy Call: " pr_instr i
  ;;
  ...
  let get_origin_dst_of_memcpy (i : instr) : value =
    ...
    else errorp "get_origin_dst_of_memcpy: not a memcopy Call: " pr_instr i
  ;;
  ...
  let predicate_of_instr_icmp (i : instr) : LL.Icmp.t option =
    ...
    | _ -> errorp "predicate: not an instr Icmp: " pr_instr i
  ;;
  ...
  let predicate_of_instr_fcmp (i : instr) : LL.Fcmp.t option =
    ...
    | _ -> errorp "predicate: not an instr FCmp: " pr_instr i
  ;;
  ...
  let branch_of_instr_br (i : instr) =
    ...
    | _ -> errorp "branch: not an instr Br: " pr_instr i
  ;;
  ...
  let src_of_instr_phi (i : instr) : values =
    ...
    | LO.PHI ->
      ...
      !operands
    | _ -> errorp "operands: not an instr PHI: " pr_instr i
  ;;
  ...
  let dst_of_instr_phi (i : instr) : value =
    ...
    | _ -> errorp "dst_of_instr: not an instr PHI: " pr_instr i
  ;;
...
module Path = struct
  ...
  let extract_icmp_predicate (cond : value) : predicate =
    ...
    | None -> errorp "extract_icmp_predicate: not Icmp cond: " pr_value cond
    | Some cmp ->
  ...
  let extract_fcmp_predicate (cond : value) : predicate =
    ...
    | None -> errorp "extract_fcmp_predicate: not Fcmp cond: " pr_value cond
    | Some cmp ->
      let lhs, rhs = LL.operand cond 0, LL.operand cond 1 in
  ...
  let extract_trunc_predicate (cond : value) : predicate =
    ...
    | LO.Trunc ->
      ...
      mk_pred_true ()
    | _ -> errorp "extract_trunc_predicate: not a Trunc cond: " pr_value cond
  ;;
  ...
  let extract_zext_predicate (cond : value) : predicate =
    ...
    | LO.ZExt ->
      ...
      mk_pred_true ()
    | _ -> errorp "extract_zext_predicate: not a ZExt cond: " pr_value cond
  ;;
...
module Func = struct
  ...
  let formal_params_of_func (f : func) : param list =
    ...
    | _ -> errorp "formal_params_of_func: not an actual function: " pr_value v
  ;;
  ...
  let get_func_used_globals (prog : program) (f : func) : globals =
    ...
    then (
      ...
      | None -> errorp "get_func_used_globals: no infor of: " func_name f
      | Some gs -> gs)
    else []
  ;;
...
module Metadata = struct
  ...
  let position_of_instr (instr : instr) : position option =
    ...
    | Some instr_dbg_info ->
      ...
      (* let _ = printp "Line: " pr_int line in *)
      (* let _ = printp "Column: " pr_int column in *)
      let filename =
        match LD.di_scope_get_file ~scope:scope_md with
        | None -> ""
        | Some file_md -> LD.di_file_get_filename ~file:file_md in
      (* let _ = printp "Filename: " pr_id filename in *)
...
module Program = struct
  ...
  let update_funcs_of_pointer (prog : program) (v : value) (funcs : funcs) =
    ...
    let _ = debugp "update func pointer of: " pr_value v in
    let _ = debugp "   new funcs: " func_names new_funcs in
  ...
  let construct_map_llvalue_to_source_name (prog : program) : unit =
    ...
    let process_instr instr =
      ...
      | LO.Call | LO.Invoke ->
        ...
        then (
          let _ = printp "instr: " pr_instr instr in
