...
module Lltype = struct
  ...
  let sexp_of_t _ = Sexp.of_string "(lltype)"
...
module Llvalue = struct
  ...
  let sexp_of_t _ = Sexp.of_string "(llvalue)"
...
module CallGraph = struct
  module Vertex = struct
    ...
    let equal f1 f2 = match (f1, f2) with Func v1, Func v2 -> v1 == v2
...
let llvalue_of_global (g : global) : llvalue = match g with Global v -> v

let llvalues_of_globals (gs : globals) : llvalues =
  List.map ~f:llvalue_of_global gs

let llvalue_of_const (c : const) : llvalue = match c with Constant v -> v

let llvalue_of_instr (i : instr) : llvalue = match i with Instr v -> v

let llvalue_of_func (f : func) : llvalue = match f with Func v -> v

let llvalue_of_param (p : param) : llvalue = match p with Param v -> v
...
let mk_expr_deref (v : llvalue) : expr = Deref (mk_expr_var v)

let mk_expr_elemptr (root : expr) (rtyp : lltype) (idxs : expr list) : expr =
  ElemPtr (root, rtyp, idxs)

let mk_expr_malloc (v : llvalue) : expr = Malloc (mk_expr_var v)

let mk_expr_func_result (f : func) : expr = FuncRes f

let mk_expr_exn (tinfo : llvalue) : expr = Exn (mk_expr_var tinfo)
...
let deref_of_expr (e : expr) : expr =
  match e with Var v -> mk_expr_deref v | _ -> Deref e
...
let equal_func (f1 : func) (f2 : func) : bool =
  match (f1, f2) with Func v1, Func v2 -> equal_llvalue v1 v2

let equal_block (b1 : block) (b2 : block) : bool = equal_llblock b1 b2

let equal_param (p1 : param) (p2 : param) : bool =
  match (p1, p2) with Param v1, Param v2 -> equal_llvalue v1 v2

let equal_instr (i1 : instr) (i2 : instr) : bool =
  match (i1, i2) with Instr v1, Instr v2 -> equal_llvalue v1 v2

let equal_global (g1 : global) (g2 : global) : bool =
  match (g1, g2) with Global v1, Global v2 -> equal_llvalue v1 v2

let equal_const (c1 : const) (c2 : const) : bool =
  match (c1, c2) with Constant v1, Constant v2 -> equal_llvalue v1 v2
...
let pr_instr (i : instr) : string = match i with Instr v -> pr_value_detail v

let pr_param ?(detailed = false) (p : param) : string =
  match p with Param v -> if detailed then pr_value_detail v else pr_value v

let pr_typed_param (p : param) : string =
  match p with Param v -> pr_type (LL.type_of v) ^ " " ^ pr_value v

let pr_global ?(detailed = false) (g : global) : string =
  match g with Global v -> if detailed then pr_value_detail v else pr_value v
...
let type_of_instr (i : instr) : lltype = LL.type_of (llvalue_of_instr i)
...
let deep_iter_instr ?(finstr : iter_instr option = None) (instr : instr) : unit =
  match finstr with None -> () | Some f -> f instr

let deep_iter_param ?(fparam : iter_param option = None) (param : param) : unit =
  match fparam with None -> () | Some f -> f param

let deep_iter_global ?(fglobal : iter_global option = None) (glob : global)
    : unit
  =
  match fglobal with None -> () | Some f -> f glob

let deep_iter_block ?(fblock : iter_block option = None)
    ?(finstr : iter_instr option = None) (blk : block)
    : unit
  =
  let blkname = LL.value_name (LL.value_of_block blk) in
  let iter () =
    LL.iter_instrs (fun i -> deep_iter_instr ~finstr (mk_instr i)) blk
  in
...
let deep_iter_func ?(ffunc : iter_func option = None)
    ?(fparam : iter_param option = None) ?(fblock : iter_block option = None)
    ?(finstr : iter_instr option = None) (func : func)
    : unit
  =
  let iter () =
    let v = llvalue_of_func func in
    LL.iter_params (fun p -> deep_iter_param ~fparam (mk_param p)) v;
    LL.iter_blocks (deep_iter_block ~fblock ~finstr) v
  in
  let open Option.Let_syntax in
  let res =
    let%bind f = ffunc in
...
let deep_iter_module ?(fglobal : iter_global option = None)
    ?(ffunc : iter_func option = None) ?(fparam : iter_param option = None)
    ?(fblock : iter_block option = None) ?(finstr : iter_instr option = None)
    (m : llmodule)
    : unit
  =
...
let deep_iter_program ?(fglobal : iter_global option = None)
    ?(ffunc : iter_func option = None) ?(fparam : iter_param option = None)
    ?(fblock : iter_block option = None) ?(finstr : iter_instr option = None)
    (prog : program)
    : unit
...
let deep_fold_instr ?(finstr : 'a fold_instr option = None) (acc : 'a)
    (instr : instr)
    : 'a
  =
  match finstr with None -> acc | Some f -> f acc instr

let deep_fold_param ?(fparam : 'a fold_param option = None) (acc : 'a)
    (param : param)
    : 'a
  =
  match fparam with None -> acc | Some f -> f acc param

let deep_fold_global ?(fglobal : 'a fold_global option = None) (acc : 'a)
    (glob : global)
    : 'a
  =
  match fglobal with None -> acc | Some f -> f acc glob

let deep_fold_block ?(fblock : 'a fold_block option = None)
    ?(finstr : 'a fold_instr option = None) (acc : 'a) (blk : block)
    : 'a
  =
  let fold () =
    LL.fold_left_instrs
...
let deep_fold_func ?(ffunc : 'a fold_func option = None)
    ?(fparam : 'a fold_param option = None)
    ?(fblock : 'a fold_block option = None)
    ?(finstr : 'a fold_instr option = None) (acc : 'a) (func : func)
    : 'a
  =
  let fold () =
    let v = llvalue_of_func func in
...
let deep_fold_module ?(fglobal : 'a fold_global option = None)
    ?(ffunc : 'a fold_func option = None)
    ?(fparam : 'a fold_param option = None)
    ?(fblock : 'a fold_block option = None)
    ?(finstr : 'a fold_instr option = None) (acc : 'a) (m : llmodule)
    : 'a
  =
  acc |> fun x ->
...
let deep_fold_program ?(fglobal : 'a fold_global option = None)
    ?(ffunc : 'a fold_func option = None)
    ?(fparam : 'a fold_param option = None)
    ?(fblock : 'a fold_block option = None)
    ?(finstr : 'a fold_instr option = None) (acc : 'a) (prog : program)
    : 'a
  =
  acc |> fun x ->
...
let int_of_const (v : llvalue) : int option =
  match LL.int64_of_const v with None -> None | Some i -> Int64.to_int i

let int64_of_const (v : llvalue) : int64 option = LL.int64_of_const v

let float_of_const (v : llvalue) : float option = LL.float_of_const v

let string_of_const (v : llvalue) : string option = LL.string_of_const v
...
let is_type_void (typ : lltype) : bool =
  match LL.classify_type typ with LT.Void -> true | _ -> false

let is_type_integer (typ : lltype) : bool =
  match LL.classify_type typ with LT.Integer -> true | _ -> false

let is_type_array (typ : lltype) : bool =
  match LL.classify_type typ with LT.Array -> true | _ -> false
...
let is_type_struct (typ : lltype) : bool =
  match LL.classify_type typ with LT.Struct -> true | _ -> false

let is_type_pointer (typ : lltype) : bool =
  match LL.classify_type typ with LT.Pointer -> true | _ -> false
...
let is_type_func (typ : lltype) : bool =
  match LL.classify_type typ with LT.Function -> true | _ -> false
      ...
      let ntyp =
        ...
        | LT.Struct ->
            let fld_idx =
              ...
              | _ -> error "get_elemptr_typ: can't get struct field idx"
            in
...
let is_llvalue_undef (v : llvalue) : bool =
  match LL.classify_value v with LV.UndefValue -> true | _ -> false
...
let is_llvalue_param (v : llvalue) : bool =
  match LL.classify_value v with LV.Argument _ -> true | _ -> false

let is_llvalue_instr (v : llvalue) : bool =
  match LL.classify_value v with LV.Instruction _ -> true | _ -> false

let is_llvalue_instr_phi (v : llvalue) : bool =
  match LL.classify_value v with LV.Instruction LO.PHI -> true | _ -> false
...
let is_llvalue_constant_expr (v : llvalue) : bool =
  match LL.classify_value v with LV.ConstantExpr -> true | _ -> false
...
let is_llvalue_integer_constant (v : llvalue) : bool =
  if LL.is_constant v then
    match LL.classify_type (LL.type_of v) with LT.Integer -> true | _ -> false
  else false

let is_llvalue_argument (v : llvalue) : bool =
  match LL.classify_value v with LV.Argument -> true | _ -> false

let is_llvalue_global (v : llvalue) : bool =
  match LL.classify_value v with LV.GlobalVariable -> true | _ -> false

let is_llvalue_function (v : llvalue) : bool =
  match LL.classify_value v with LV.Function -> true | _ -> false
...
let is_expr_null (e : expr) : bool =
  match e with Var v -> LL.is_null v | _ -> false

let is_expr_global (e : expr) : bool =
  match e with Var v -> is_llvalue_global v | _ -> false

let is_expr_function (e : expr) : bool =
  match e with Var v -> is_llvalue_function v | _ -> false

let is_expr_func_res (e : expr) : bool =
  match e with FuncRes _ -> true | _ -> false

let is_expr_undef (e : expr) : bool =
  match e with Var v -> is_llvalue_undef v | _ -> false

let is_expr_deref (e : expr) : bool =
  match e with Deref _ -> true | _ -> false

let is_expr_elemptr (e : expr) : bool =
  match e with ElemPtr _ -> true | _ -> false

let is_expr_malloc (e : expr) : bool =
  match e with Malloc _ -> true | _ -> false

let is_expr_var (e : expr) : bool = match e with Var v -> true | _ -> false

let is_integer_expr (e : expr) : bool =
  match e with Int64 _ -> true | _ -> false

let is_float_expr (e : expr) : bool =
  match e with Int64 _ -> true | _ -> false

let is_constant_expr (e : expr) : bool =
  match e with Int64 _ | Float _ -> true | _ -> false

let is_zero_expr (e : expr) : bool =
  match e with Int64 i -> Int64.( = ) i Int64.zero | _ -> false
...
let is_pred_true (p : predicate) : bool =
  match p with PBool true -> true | _ -> false

let is_pred_false (p : predicate) : bool =
  match p with PBool false -> true | _ -> false
...
let is_instr_load (i : instr) : bool =
  match instr_opcode i with LO.Load -> true | _ -> false

let is_instr_store (i : instr) : bool =
  match instr_opcode i with LO.Store -> true | _ -> false

let is_instr_gep (i : instr) : bool =
  match instr_opcode i with LO.GetElementPtr -> true | _ -> false

let is_instr_extractvalue (i : instr) : bool =
  match instr_opcode i with LO.ExtractValue -> true | _ -> false

let is_instr_call (i : instr) : bool =
  match instr_opcode i with LO.Call -> true | _ -> false

let is_instr_invoke (i : instr) : bool =
  match instr_opcode i with LO.Invoke -> true | _ -> false
...
let is_instr_return (i : instr) : bool =
  match instr_opcode i with LO.Ret -> true | _ -> false

let is_instr_bitcast (i : instr) : bool =
  match instr_opcode i with LO.BitCast -> true | _ -> false
...
let is_instr_unreachable (i : instr) : bool =
  match instr_opcode i with LO.Unreachable -> true | _ -> false
...
let is_func_real_func (f : func) : bool =
  let v = llvalue_of_func f in
  match LL.classify_value v with LV.Function _ -> true | _ -> false
...
let subst_value (sst : substv) (v : llvalue) : llvalue =
  ...
  match res with Some (_, b) -> b | None -> v

let subst_value_expr (sst : substve) (v : llvalue) : expr =
  let res = List.find ~f:(fun (a, b) -> equal_llvalue a v) sst in
  match res with Some (_, b) -> b | None -> Var v
...
let num_uses (v : llvalue) : int = LL.fold_left_uses (fun acc _ -> acc + 1) 0 v
...
let mk_pred_conj (ps : predicate list) : predicate =
  ...
  if List.is_empty ps then mk_pred_false ()
    ...
    if List.exists ~f:is_pred_false nps then mk_pred_false ()
      ...
      match nps with [] -> mk_pred_true () | [ np ] -> np | _ -> PConj nps
...
let mk_pred_disj (ps : predicate list) : predicate =
  ...
  if List.is_empty ps then mk_pred_true ()
    ...
    if List.exists ~f:is_pred_true nps then mk_pred_true ()
      ...
      match nps with [] -> mk_pred_false () | [ np ] -> np | _ -> PDisj nps
...
let get_struct_types (m : llmodule) : lltype list =
  ...
  let rec collect_struct_type typ =
    if is_type_struct typ then
      ...
      List.iter ~f:collect_struct_type subtypes
  in
  let fglobal =
...
let get_origin_src_of_memcpy (i : instr) : llvalue =
  ...
  if is_func_memcpy callee then operand (mk_instr (operand i 0)) 0
  else herror "get_origin_src_of_memcpy: not a memcopy Call: " pr_instr i

let get_origin_dst_of_memcpy (i : instr) : llvalue =
  let callee = callee_of_instr_func_call i in
  if is_func_memcpy callee then operand (mk_instr (operand i 1)) 0
...
let is_entry_block_of_function (blk : block) : bool =
  match LL.block_pred blk with LL.At_start _ -> true | LL.After _ -> false
...
let get_pathcond_between_blocks prog (src : block) (dst : block)
  ...
  let sblks =
    List.filter ~f:(fun sblk -> equal_block sblk.sblk_block dst) sblks
  in
  match sblks with [] -> None | sblk :: _ -> Some sblk.sblk_pathcond
  ...
  let rec compute_reachables (queue : blocks) (visited : blocks) =
    ...
    | blk :: nqueue ->
        let nblks =
          ...
          |> List.exclude ~f:(List.mem ~equal:( == ) nqueue)
        in
        let nqueue = nqueue @ nblks in
        let nvisited = visited @ [ blk ] in
        compute_reachables nqueue nvisited
  in
  let compute () =
    let sblks =
      blk |> get_succeeding_blocks prog |> List.map ~f:(fun sb -> sb.sblk_block)
    in
    compute_reachables sblks []
  in
...
let is_reachable_instr prog (src : instr) (dst : instr) : bool =
  let rec check_reachable_instr_same_block instr1 instr2 =
    if equal_instr instr1 instr2 then true
      ...
      | None -> false
      | Some instr' -> check_reachable_instr_same_block instr' instr2
  in
  let blk1, blk2 = (block_of_instr src, block_of_instr dst) in
  if equal_block blk1 blk2 then check_reachable_instr_same_block src dst
...
let mk_program (filename : string) (m : llmodule) : program =
  ...
  let globals =
    LL.fold_left_globals (fun acc g -> acc @ [ mk_global g ]) [] m
  in
...
let pr_loop (l : loop) : string =
  "Loop: {head: " ^ block_name l.loop_head ^ "; body: "
...
let pr_func (f : func) : string =
  ...
  let sblks =
    ...
    |> String.replace_if_empty ~replacer:"{Empty function}"
  in
  fname ^ "\n" ^ sblks
...
let pr_caller_info (prog : program) : string =
  Hashtbl.fold
    ~f:(fun ~key:func ~data:callers acc ->
      ...
      let caller_names =
        callers |> List.map ~f:func_name |> String.concat ~sep:", "
      in
      acc ^ "\n  " ^ fname ^ " <-- [" ^ caller_names ^ "]")
...
let pr_callee_info (prog : program) : string =
  Hashtbl.fold
    ~f:(fun ~key:func ~data:callees acc ->
      let fname = func_name func in
      let callee_names =
        callees |> List.map ~f:func_name |> String.concat ~sep:", "
      in
      acc ^ "\n  " ^ fname ^ " --> [" ^ callee_names ^ "]")
