...
let choose_rule_all_pure prog goal : rule list =
  ...
  then (
    ...
    if is_true res then [ mk_rule_all_pure goal.gl_entail_cores ] else [])
  else []
;;
...
let choose_rule_infer_frame prog goal : rule list =
  ...
  | [ enc ] ->
    ...
    then (
      ...
      if is_true (SMT.check_imply plhs prhs)
...
let sprint_rule_result ?(print = true) pstate rule encs =
  ...
  let nencs =
    List.map
      ~f:(fun (enc, eqs) ->
        ...
        sprint_rule_result pstate rule [ enc; nenc ];
        nenc)
    ...
    | enc :: nencs ->
      let lhs, changed =
        ...
        | Pure (Reln ((RName rn, _) as rf)) ->
          ...
          | f -> herror "rule_unfold_head: not pure reln body" sprint_f f)
        | _ -> renc.enc_lhs, false in
      let nenc = update_entail_core enc ~lhs:[ lhs ] in
      sprint_rule_result ~print:changed pstate rule [ renc; enc; nenc ];
