...
let choose_rule_no_entail prog goal : rule list =
  if List.is_empty goal.gl_entail_cores then [mk_rule_no_entail ()]
  else []

let choose_rule_all_pure prog goal : rule list =
  let pure_entails = List.filter ~f:(fun enc ->
      let lst, rst = enc.enc_lst, enc.enc_rst in
      lst.fst_is_pure && rst.fst_is_pure) goal.gl_entail_cores in
  if pure_entails != [] &&
     List.length pure_entails = List.length goal.gl_entail_cores then
    let ents = get_entailments_of_goal goal in
    let res, _ = SMT.check_sat_horn ents in
    if is_true res then
      [mk_rule_all_pure goal.gl_entail_cores]
    else []
  else []
...
let choose_rule_false_left prog goal : rule list =
  let encs = List.filter ~f:(fun enc ->
    let nlhs, changed = simplify_tauto_contra_f enc.enc_lhs in
    changed && (is_f_false nlhs)) goal.gl_entail_cores in
  if List.is_empty encs then []
  else [mk_rule_false_left encs]

let choose_rule_valid_entails prog goal : rule list =
  let encs = List.filter ~f:(fun enc ->
    let lhs, rhs = enc.enc_lhs, enc.enc_rhs in
    let lst, rst = enc.enc_lst, enc.enc_rst in
    if lst.fst_is_pure && rst.fst_is_pure then
      let plhs = extract_pure_form lhs in
      let prhs = extract_pure_form rhs in
      is_true (SMT.check_imply plhs prhs)
    else false) goal.gl_entail_cores in
  if encs != [] then [mk_rule_valid_entails encs]
  else []

let choose_rule_infer_frame prog goal : rule list =
  match goal.gl_entail_cores with
  | [enc] ->
    if enc.enc_rst.fst_is_pure then
      let plhs = encode_formula_to_pure enc.enc_lhs in
      let prhs = extract_pure_form enc.enc_rhs in
      if is_true (SMT.check_imply plhs prhs) then [mk_rule_infer_frame enc]
      else []
    else []
  | _ -> []

let choose_rule_elim_bvar prog goal : rule list =
  let encs_eqss = List.fold_left ~f:(fun acc enc ->
    let eqs = enc.enc_lhs |> collect_eq_exp_f |>
              List.filter ~f:(function | EqPure _ -> true | _ -> false) in
    if List.is_empty eqs then acc
    else acc @ [(enc, eqs)]) ~init:[] goal.gl_entail_cores in
  if List.is_empty encs_eqss then []
  else [(mk_rule_elim_bvar encs_eqss)]

let choose_rule_equal_left prog goal : rule list =
  let encs_eqss = List.fold_left ~f:(fun acc enc ->
    let eqs = enc.enc_lhs |> collect_eq_exp_f |>
              List.filter ~f:(function | EqExp _ -> true | _ -> false) in
    if List.is_empty eqs then acc
    else acc @ [(enc, eqs)]) ~init:[] goal.gl_entail_cores in
  if List.is_empty encs_eqss then []
  else [(mk_rule_equal_left encs_eqss)]

let choose_rule_exists_left prog goal : rule list =
  let can_subst_vars vs eqs = List.exists ~f:(function
  ...
  let rec need_simplify_f f = match f with
  ...
  let encs = List.filter ~f:(fun enc ->
    need_simplify_f enc.enc_lhs) goal.gl_entail_cores in
  if List.is_empty encs then []
  else [mk_rule_exists_left encs]
...
let choose_rule_exists_right prog goal : rule list =
  ...
  let rec need_simplify_f f = match f with
    ...
    | Exists (vs, g) ->
      (need_simplify_f g) || (can_subst_vars vs (collect_eq_exp_f g)) in
  let encs = List.filter ~f:(fun enc ->
    need_simplify_f enc.enc_rhs) goal.gl_entail_cores in
  if List.is_empty encs then []
  else [mk_rule_exists_right encs]

let choose_rule_reln_left prog goal : rule list =
  let rec find_rule encs = match encs with
    | [] -> []
    | enc::encs ->
      let rnames = collect_reln_name_f enc.enc_lhs in
      try
        let rdefn = List.find_exn ~f:(fun rd ->
          SP.exists ~f:(String.equal rd.relnd_name) rnames) prog.prog_reln_defns in
...
let choose_rule_reln_right prog goal : rule list =
  let rec find_rule encs = match encs with
    | [] -> []
    | enc::encs -> match enc.enc_rhs with
...
let choose_rule_wand_inner prog goal : rule list =
  ...
  let rec find_elim_form f = match f with
    | Emp | Pure _ | Data _ | View _
    | Iter _ | Array _ -> []
    ...
    | Star fs ->
      ...
      let gs =
        try
          let f = List.find_exn ~f:(fun f1 -> List.exists ~f:(function
  ...
  let encs = List.fold_left ~f:(fun acc e ->
    match find_elim_form e.enc_lhs with
    | [] -> acc
    | fs -> acc @ [(e, fs)]) ~init:[] goal.gl_entail_cores in
  if List.is_empty encs then []
  else [mk_rule_wand_inner encs]
...
let choose_rule_wand_outer prog goal : rule list =
  ...
  let rec find_elim_form f = match f with
    ...
    | Wand (Data df as fd, g) ->
      let fs = match g with | Star fs -> fs | _ -> [g] in
...
let choose_rule_wand_data prog goal : rule list =
  ...
  let encs = List.fold_left ~f:(fun acc e ->
    ...
    | fs -> acc @ [(e, fs)]) ~init:[] goal.gl_entail_cores in
  if List.is_empty encs then []
  else [mk_rule_wand_data encs]

let choose_rule_wand_right prog goal : rule list =
  (* Rule: P |- Q --* R  <==>  P * Q |- R *)
  let encs = List.filter ~f:(fun e ->
...
let choose_rule_unfold_head prog goal : rule list =
  let rec find_rule encs used_encs rs = match encs with
    | [] -> rs
    | enc::nencs ->
...
let choose_rule_unfold_view_left prog goal : rule list =
  List.fold ~f:(fun acc1 enc ->
    List.fold ~f:(fun acc2 vsp ->
      let vf, rest, evs = vsp.vsp_head, vsp.vsp_rest, vsp.vsp_evars in
      if has_view_defn prog vsp.vsp_head.view_name then
        acc2 @ [mk_rule_unfold_view_left vf rest evs enc]
      else acc2
    ) ~init:acc1 enc.enc_lst.fst_view_splits
  ) ~init:[] goal.gl_entail_cores

let choose_rule_unfold_view_right prog goal : rule list =
  List.fold ~f:(fun arules enc ->
    List.fold ~f:(fun acc vsp ->
      let vf, rest, evs = vsp.vsp_head, vsp.vsp_rest, vsp.vsp_evars in
      if has_view_defn prog vsp.vsp_head.view_name then
        acc @ [mk_rule_unfold_view_left vf rest evs enc]
      else acc
...
let choose_rule_match_data prog goal : rule list =
  List.fold_left ~f:(fun arules enc ->
    List.fold_left ~f:(fun acc1 ldsp ->
      List.fold_left ~f:(fun acc2 rdsp ->
        let ldf, rdf = ldsp.dsp_head, rdsp.dsp_head in
        if (equal_typ ldf.data_typ rdf.data_typ) then
          acc2 @ [mk_rule_match_data enc ldsp rdsp]
        else acc2
...
let choose_rule_match_view prog goal : rule list =
  List.fold_left ~f:(fun arules enc ->
    List.fold_left ~f:(fun acc1 lvsp ->
      List.fold_left ~f:(fun acc2 rvsp ->
        let ldf, rdf = lvsp.vsp_head, rvsp.vsp_head in
        if (String.equal ldf.view_name rdf.view_name) then
          acc2 @ [mk_rule_match_view enc lvsp rvsp]
        else acc2
...
let choose_rule_match_array prog goal : rule list =
  List.fold_left ~f:(fun arules enc ->
    List.fold_left ~f:(fun acc1 lasp ->
      List.fold_left ~f:(fun acc2 rasp ->
        let laf, raf = lasp.asp_head, rasp.asp_head in
        if (equal_exp laf.array_root raf.array_root) then
          acc2 @ [mk_rule_match_array enc lasp rasp]
        else acc2
    ...
    List.fold_left ~f:(fun acc1 lasp ->
      ...
      List.fold_left ~f:(fun acc2 rdsp ->
        ...
        let has_same_root_df =
          let update_cond = List.exists ~f:(fun d ->
            equal_exp d.data_root rdsp.dsp_head.data_root) laf.array_update in
          let root_cond = equal_exp laf.array_root rdf.data_root in
          update_cond || root_cond in
        if has_same_root_df then
          acc2 @ [mk_rule_subtract_data enc lasp rdsp]
        else acc2
      ) ~init:acc1 enc.enc_rst.fst_data_splits
    ) ~init:arules enc.enc_lst.fst_array_splits
  ) ~init:[] goal.gl_entail_cores
...
let choose_rule_empty_array_right prog goal : rule list =
  List.fold_left ~f:(fun arules enc ->
    List.fold_left ~f:(fun acc rasp ->
      let raf, rest, evs = rasp.asp_head, rasp.asp_rest, rasp.asp_evars in
      let has_data_same_root = List.exists ~f:(fun ldsp ->
        equal_exp ldsp.dsp_head.data_root raf.array_root
      ) enc.enc_lst.fst_data_splits in
      let has_array_same_root = List.exists ~f:(fun lasp ->
        equal_exp lasp.asp_head.array_root raf.array_root
      ) enc.enc_lst.fst_array_splits in
      if not has_data_same_root && not has_array_same_root then
        acc @ [mk_rule_empty_array_right enc raf rest evs]
      else acc
...
let pr_rule_result ?(print=true) pstate rule encs =
  ...
  if pstate.prs_interact && print then
    let sencs = encs |> List.map ~f:(fun c -> "  " ^ (pr_enc ~id:true c)) |>
                String.concat ~sep:"\n  ==>\n" in
    debugc ("@ " ^ action ^ ":\n" ^ sencs)
  else ()
...
let process_rule_false_left pstate goal r : derivation =
  ...
  let nencs = List.map ~f:(fun enc ->
    if List.exists ~f:(fun h -> h.enc_id = enc.enc_id) r.rfl_entails then
      update_entail_core enc ~lhs:[mk_f_false ()]
    else enc) goal.gl_entail_cores in
...
let process_rule_elim_bvar pstate goal r : derivation =
  ...
  let rec elim_p (sst: var -> pure_form) p = match p with
    | Bool _ | Reln _ -> p
    | BExp (Var v) -> sst v
    | BExp _ -> p
    | BEq (e, p) -> mk_pf_true ()
  ...
  and elim_f (sst: var -> pure_form) f = match f with
  ...
  let nencs = List.map ~f:(fun (enc, eqs) ->
    let sst0 = (fun v -> mk_bexp (mk_exp_var v)) in
    let sst = List.fold_left ~f:(fun acc eq -> match eq with
      | EqExp _ -> acc
      | EqPure (v, p) ->
        fun u -> if equal_var u v then p else acc u) ~init:sst0 eqs in
...
let process_rule_wand_inner pstate goal r : derivation =
  ...
  let rec eliminate f = match f with
    ...
    | Iter _ | Array _ -> f
    | Star fs ->
      let fs1, fs2 = fs |> List.map ~f:eliminate |>
                     List.partition_tf ~f:is_f_data_or_view in
      let rec remove fs acc = match fs with
        | [] -> mk_f_star (acc @ fs @ fs2)
        | f::fs ->
          let fs21, fs22 = List.partition_tf ~f:(function
  ...
  let encs = List.map ~f:(fun (enc, fs) ->
    let lhs = eliminate enc.enc_lhs in
    let nenc = update_entail_core enc ~lhs:[lhs] in
    pr_rule_result pstate rule [enc; nenc];
    nenc) r.rwi_entails in
  let sub_goal = update_goal_entcores goal encs in
  mk_derivation_subgoals goal [sub_goal] rule
...
let process_rule_wand_outer pstate goal r : derivation =
  ...
  let rec eliminate f = match f with
    ...
    | Wand (f1, f2) ->
      ...
      (match f1 with
       | Data df as fd ->
         ...
         if List.is_empty fs1 then f
         else mk_f_star ((List.tl_exn fs1) @ fs2)
       | View vf as fv ->
         let fs = match f2 with | Star fs -> fs | _ -> [f2] in
    ...
    | Exists (vs, f) -> mk_f_exists vs (eliminate f) in
  let encs = List.map ~f:(fun (enc, _) ->
    let lhs = eliminate enc.enc_lhs in
    let nenc = update_entail_core enc ~lhs:[lhs] in
    pr_rule_result pstate rule [enc; nenc];
    nenc) r.rwo_entails in
  let sub_goal = update_goal_entcores goal encs in
  mk_derivation_subgoals goal [sub_goal] rule
...
let process_rule_wand_right pstate goal r : derivation =
  ...
  let nenc = match enc.enc_rhs with
    | Wand (f1, f2) ->
...
let process_rule_unfold_head pstate goal r : derivation =
  ...
  let rec process encs acc = match encs with
    ...
    | enc::nencs ->
      let lhs, changed = match renc.enc_rhs with
        | View vf ->
          let vnames = collect_view_name_f enc.enc_lhs in
          if not (SP.mem vnames vf.view_name) then (enc.enc_lhs, false)
          else (replace_view_form_f vf renc.enc_lhs enc.enc_lhs, true)
        | Pure (Reln ((RName rn, _) as rf)) -> (match renc.enc_lhs with
          | Pure p ->
            let rnames = collect_reln_name_f enc.enc_lhs in
            if not (SP.mem rnames rn) then (enc.enc_lhs, false)
...
let process_rule_match_data pstate goal r : derivation =
  ...
  let rec process encs acc = match encs with
    | [] -> acc
    | enc::encs ->
      if enc.enc_id = r.rmd_entail.enc_id then
        let rhs = r.rmd_rhs_rest |> mk_f_star_with ~pfs:[r.rmd_matching_form] |>
...
let process_rule_match_view pstate goal r : derivation =
  ...
  let rec process encs acc = match encs with
    | [] -> acc
    | enc::encs ->
      if enc.enc_id = r.rmv_entail.enc_id then
        let rhs = r.rmv_rhs_rest |> mk_f_star_with ~pfs:[r.rmv_matching_form] |>
...
let process_rule_match_array pstate goal r : derivation =
  ...
  let rec process encs acc = match encs with
    | [] -> acc
    | enc::encs ->
      if enc.enc_id = r.rma_entail.enc_id then
        let rhs = r.rma_rhs_rest |> mk_f_star_with ~pfs:[r.rma_matching_form] |>
...
let compare_rule_unfold_head_vs_others pstate goal r1 r2 =
  ...
  | RlMatchData r2 ->
    compare_rule_unfold_head_vs_match_data pstate goal r1 r2
  | RlMatchView r2 ->
    compare_rule_unfold_head_vs_match_view pstate goal r1 r2
  | RlMatchArray r2 ->
  ...
  | _ ->
    herror "compare_rule_unfold_head_vs_others: not expect" pr_rule r2
...
let compare_rule_unfold_view_right_vs_unfold_view_left pstate goal r1 r2 =
  neg_prio (compare_rule_unfold_view_left_vs_unfold_view_right pstate goal r2 r1)
...
let compare_rule_match_data_vs_match_data pstate goal r1 r2 =
  try
    if r1.rmd_same_root && r2.rmd_same_root then
      raise_prio PrioEqual;
    if r1.rmd_same_root && not r2.rmd_same_root then
      raise_prio PrioHigh;
    if not r1.rmd_same_root && r2.rmd_same_root then
      raise_prio PrioLow;
    (* default *)
    PrioEqual
  with EPrio p -> p
...
let compare_rule_match_data_vs_others pstate goal r1 r2 =
  match r2 with
  | RlUnfoldHead r2 ->
    compare_rule_match_data_vs_unfold_head pstate goal r1 r2
  | RlUnfoldViewLeft r2 ->
  ...
  | RlMatchData r2 ->
    compare_rule_match_data_vs_match_data pstate goal r1 r2
  | RlMatchView r2 ->
    compare_rule_match_data_vs_match_view pstate goal r1 r2
  | RlMatchArray r2 ->
    compare_rule_match_data_vs_match_array pstate goal r1 r2
  | RlSubtractData r2 ->
...
let compare_rule_match_view_vs_others pstate goal r1 r2 =
  match r2 with
  | RlUnfoldHead r2 ->
    compare_rule_match_view_vs_unfold_head pstate goal r1 r2
  | RlUnfoldViewLeft r2 ->
  ...
  | RlMatchData r2 ->
    compare_rule_match_view_vs_match_data pstate goal r1 r2
  | RlMatchView r2 ->
    compare_rule_match_view_vs_match_view pstate goal r1 r2
  | RlMatchArray r2 ->
    compare_rule_match_view_vs_match_array pstate goal r1 r2
  | RlSubtractData r2 ->
    compare_rule_match_view_vs_subtract_data pstate goal r1 r2
  | RlEmptyArrayRight r2 ->
    compare_rule_match_view_vs_empty_array_right pstate goal r1 r2
  | _ ->
    herror "compare_rule_match_view_vs_others: not expect" pr_rule r2
...
let compare_rule_match_array_vs_others pstate goal r1 r2 =
  ...
  | RlMatchData r2 ->
    compare_rule_match_array_vs_match_data pstate goal r1 r2
  | RlMatchView r2 ->
    compare_rule_match_array_vs_match_view pstate goal r1 r2
  | RlMatchArray r2 ->
...
let compare_rule_empty_array_right_vs_unfold_view_left pstate goal r1 r2 =
  neg_prio (compare_rule_unfold_view_left_vs_empty_array_right pstate goal r2 r1)

let compare_rule_empty_array_right_vs_unfold_view_right pstate goal r1 r2 =
  neg_prio (compare_rule_unfold_view_right_vs_empty_array_right pstate goal r2 r1)
...
let compare_transformation_rules pstate goal r1 r2 =
  match r1 with
  | RlUnfoldHead r1 ->
    compare_rule_unfold_head_vs_others pstate goal r1 r2
  | RlUnfoldViewLeft r1 ->
  ...
  | RlMatchData r1 ->
    compare_rule_match_data_vs_others pstate goal r1 r2
  | RlMatchView r1 ->
    compare_rule_match_view_vs_others pstate goal r1 r2
  | RlMatchArray r1 ->
    compare_rule_match_array_vs_others pstate goal r1 r2
  | RlSubtractData r1 ->
    compare_rule_subtract_data_vs_others pstate goal r1 r2
