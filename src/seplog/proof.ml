...
type rule_pure_entails = {
  rap_entails : entail_cores;
}

type rule_invalid_entail = {
  rie_entail : entail_core;
}

(* normalization rules *)

type rule_false_left = {
  rfl_entails : entail_cores;
}

type rule_valid_entails = {
  rve_entails : entail_cores;
}

type rule_infer_frame = {
  rif_entail : entail_core;
}

type rule_elim_bvar = {
  reb_entails_eqss : (entail_core * equalities) list;
}

type rule_equal_left = {
  reql_entails_eqss : (entail_core * equalities) list;
}

type rule_exists_left = {
  rexl_entails : entail_cores;
}

type rule_exists_right = {
  rexr_entails : entail_cores;
}
...
type rule_wand_inner = {
  rwi_entails : (entail_core * formula list) list;
}

type rule_wand_outer = {
  rwo_entails : (entail_core * formula list) list;
}

type rule_wand_data = {
  rwd_entails : (entail_core * formula list) list;
}

type rule_wand_right = {
  rwr_entail : entail_core;
}
...
type rule_unfold_head = {
  ruh_entail : entail_core;
}
...
let neg_prio p = match p with
  | PrioHigh -> PrioLow
  | PrioEqual -> PrioEqual
  | PrioLow -> PrioHigh

let pr_prio p = match p with
  | PrioHigh -> "High"
  | PrioEqual -> "Equal"
  | PrioLow -> "Low"

let encode_prio p = match p with
  | PrioHigh -> 1
  | PrioEqual -> 0
  | PrioLow -> -1
...
let pr_enc ?(id=false) enc =
  ...
  if not id || enc.enc_id < 1 then "# " ^ res
  else "#" ^ (pr_int enc.enc_id) ^ ". " ^ res

let pr_enc_id (enc: entail_core) : string =
  "#" ^ (pr_int enc.enc_id)
...
let pr_ric r =
  "Invalid Entail {" ^ (pr_enc_id r.rie_entail) ^ "}"

(* normalization *)

let pr_rfl r =
  "False Left {" ^ (pr_enc_ids r.rfl_entails) ^ "}"

let pr_rve r =
  "Valid Entails {" ^ (pr_enc_ids r.rve_entails) ^ "}"

let pr_rif r =
  "Infer Frame {" ^ (pr_enc_id r.rif_entail) ^ "}"

let pr_reb r =
  let encs = List.fold_left ~f:(fun acc (enc, eqs) ->
...
let pr_rexl r =
  "Exists Left {" ^ (pr_enc_ids r.rexl_entails) ^ "}"

let pr_rexr r =
  "Exists Right {" ^ (pr_enc_ids r.rexr_entails) ^ "}"

let pr_rrl r =
  "Reln Left {" ^ (pr_enc_id r.rrl_entail) ^ ": " ^
  r.rrl_rdefn.relnd_name ^ "}"

let pr_rrr r =
  "Reln Right {" ^ (pr_enc_id r.rrr_entail) ^ ": " ^
  r.rrr_rdefn.relnd_name ^ "}"

let pr_rwi r =
  let encs = List.fold_left ~f:(fun acc (enc, fs) ->
    let acc = if (String.equal acc "") then acc else acc ^ ", " in
...
let pr_rwr r =
  "Wand Right {" ^ (pr_enc_id r.rwr_entail) ^ "}"
...
let pr_rmd r =
  "Match Data {" ^ (pr_enc_id r.rmd_entail) ^ ": " ^
  (pr_df r.rmd_lhs_data) ^ " vs. " ^ (pr_df r.rmd_rhs_data) ^
  ", early: " ^ (pr_bool r.rmd_apply_early) ^ "}"

let pr_rmv r =
  "Match View {" ^ (pr_enc_id r.rmv_entail) ^ ": " ^
  (pr_vf r.rmv_lhs_view) ^ " vs. " ^ (pr_vf r.rmv_rhs_view) ^
  ", early: " ^ (pr_bool r.rmv_apply_early) ^ "}"

let pr_rma r =
  "Match Array {" ^ (pr_enc_id r.rma_entail) ^ ": " ^
  (pr_af r.rma_lhs_array) ^ " vs. " ^ (pr_af r.rma_rhs_array) ^
  ", early: " ^ (pr_bool r.rma_apply_early) ^ "}"

let pr_rsd r =
  "Subtract Data {" ^ (pr_enc_id r.rsd_entail) ^ ": " ^
  (pr_af r.rsd_lhs_array) ^ " vs. " ^ (pr_df r.rsd_rhs_data)
...
let pr_rules rules =
  let res = List.fold_left ~f:(fun acc r ->
    acc ^ "\n  - " ^ (pr_rule r)) ~init:"" rules in
  "[" ^ res ^ "]"
...
let pr_goal goal =
  let encs = List.fold_left ~f:(fun acc enc ->
    acc ^ "   #" ^ (pr_int enc.enc_id) ^ ". " ^
    (pr_f enc.enc_lhs) ^ " |- " ^ (pr_f enc.enc_rhs) ^ "\n")
    ~init:"" goal.gl_entail_cores in
  let traces =
    if List.is_empty goal.gl_trace then "[]"
    else goal.gl_trace |> List.map ~f:pr_rule |>
         String.concat ~sep:"\n     ==> " in
  " @ Entails:\n" ^ encs ^
  " @ Trace: " ^ traces
...
let mk_rule_no_entail () =
  RlNoEntail

let mk_rule_all_pure encs =
  RlAllPure {rap_entails = encs}

let mk_rule_invalid_entail enc =
  RlInvalidEntail {rie_entail = enc}

(* normalization rules *)

let mk_rule_false_left encs =
  RlFalseLeft {rfl_entails = encs}

let mk_rule_valid_entails encs =
  RlValidEntails {rve_entails = encs}

let mk_rule_infer_frame enc =
  RlInferFrame {rif_entail = enc}

let mk_rule_elim_bvar ents_eqss =
  RlElimBVar {reb_entails_eqss = ents_eqss}

let mk_rule_equal_left ents_eqss =
  RlEqualLeft {reql_entails_eqss = ents_eqss}

let mk_rule_exists_left encs =
  RlExistsLeft {rexl_entails = encs}

let mk_rule_exists_right encs =
  RlExistsRight {rexr_entails = encs}

let mk_rule_reln_left rd enc =
  RlRelnLeft {rrl_rdefn = rd; rrl_entail = enc}

let mk_rule_reln_right rd enc =
  RlRelnRight {rrr_rdefn = rd; rrr_entail = enc}

let mk_rule_wand_inner encs =
  RlWandInner {rwi_entails = encs}

let mk_rule_wand_outer encs =
  RlWandOuter {rwo_entails = encs}

let mk_rule_wand_data encs =
  RlWandData {rwd_entails = encs}

let mk_rule_wand_right enc =
  RlWandRight {rwr_entail = enc}
...
let mk_rule_unfold_head enc =
  RlUnfoldHead {ruh_entail = enc}
...
let mk_matching_forms args1 args2 =
  let equal_args = List.fold2_exn ~f:(fun acc x y ->
    if equal_exp x y then acc
    else acc @ [mk_eq x y]) ~init:[] args1 args2 in
  mk_pconj equal_args
...
let mk_rule_match_array enc lasp rasp =
  ...
  let matching_form =
    ...
    mk_matching_forms largs rargs in
  let has_same_args = (equal_exp laf.array_root raf.array_root) &&
...
let mk_rule_subtract_data enc lasp rdsp =
  ...
  let matched_dfs, other_update = List.partition_tf ~f:(fun d ->
    equal_exp d.data_root rdf.data_root) laf.array_update in
  let matched_df = match matched_dfs with
    | [] ->
      let nargs = List.map ~f:(fun e -> fresh_exp_var (typ_of_exp e))
                    rdf.data_args in
...
let split_one_data_form (f: formula) : data_split list =
  ...
  let rec split_f f : (data_form * formula * vars) list =
    match f with
    | Pure _ | Emp | View _ | Iter _ | Array _
    | Wand _ | Septract _ -> []
    | Data df -> [(df, Emp, [])]
    | Star fs ->
      fs |> extract [] [] |>
      List.map ~f:(fun (df, fs) -> (df, mk_f_star fs, []))
    | Exists (vs, f) ->
...
let split_one_view_form (f: formula) : view_split list =
  ...
  let rec split_f f : (view_form * formula * vars) list =
    ...
    | Star fs ->
      fs |> extract [] [] |>
      List.map ~f:(fun (vf, fs) -> (vf, mk_f_star fs, []))
    | Exists (vs, f) ->
...
let split_one_array_form (f: formula) : array_split list =
  ...
  let rec split_f f : (array_form * formula * vars) list =
    ...
    | Star fs ->
      fs |> extract [] [] |>
      List.map ~f:(fun (af, fs) -> (af, mk_f_star fs, []))
    | Exists (vs, f) ->
...
let update_goal_entcores goal encs : goal =
  let encs = List.map ~f:(fun enc ->
    ...
    with _ -> enc) goal.gl_entail_cores in
  {goal with gl_entail_cores = encs}

let remove_goal_entcores goal encs : goal =
  let nents = List.fold_left ~f:(fun acc enc ->
    if List.exists ~f:(fun e -> enc.enc_id = e.enc_id) encs then acc
    else acc @ [enc]) ~init:[] goal.gl_entail_cores in
  {goal with gl_entail_cores = nents}

let mk_derivation_subgoals goal sub_goals rule =
  let sub_goals = List.map ~f:(fun g ->
    {g with gl_trace = g.gl_trace @ [rule]}) sub_goals in
  {drv_goal = goal;
   drv_kind = DrvSubgoals sub_goals;
   drv_rule = rule;}
...
let mk_proof_tree_valid goal rule subtrees : proof_tree =
  let frames = List.fold_left ~f:(fun acc pt ->
    acc @ pt.ptr_frames) ~init:[] subtrees in
  let frames = match rule with
    | RlInferFrame r ->
      [r.rif_entail.enc_lhs] @ frames
    | RlMatchData r ->
      List.map ~f:(fun f -> mk_f_star_with f ~pfs:[r.rmd_matching_form]) frames
    | RlMatchView r ->
      List.map ~f:(fun f -> mk_f_star_with f ~pfs:[r.rmv_matching_form]) frames
    | RlMatchArray r ->
      List.map ~f:(fun f -> mk_f_star_with f ~pfs:[r.rma_matching_form]) frames
    | RlEmptyArrayRight r ->
...
let mk_prover_state prog : prover_state =
  {prs_prog = prog;
   prs_interact = !mode_interactive_prover;}
