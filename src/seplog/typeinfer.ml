...
let reset_tvar_index () =
  tvar_index := 0

let new_typ_env () : environment =
  fun x -> TUnk
...
let remove_vars_from_env (env: environment) (vs: var list) : environment =
  ...
  (fun n ->
     if List.mem ~equal:String.equal ns n then TUnk
     else env n)
...
let update_substitution sst (old_typ: typ) (new_typ: typ) : substitution =
  ...
  let _ =
    if is_typ_known cur_typ && is_typ_known new_typ &&
       not (equal_typ cur_typ new_typ) then
    error ("update_sst: unmatched current and new types: " ^
           (pr_type cur_typ) ^ " ~~ " ^ (pr_type new_typ)) in
  fun t ->
    let substed_t = sst t in
    match cur_typ with
    | TVar _ ->
      if equal_typ t old_typ || equal_typ t cur_typ ||
         equal_typ substed_t old_typ || equal_typ substed_t cur_typ then
        new_typ
      else substed_t
    | _ ->
      if equal_typ t old_typ || equal_typ t new_typ ||
         equal_typ substed_t old_typ || equal_typ substed_t new_typ then
        cur_typ
      else substed_t

let unify_typ sst current_typ expected_typ : substitution =
  if equal_typ current_typ expected_typ then sst
  else match current_typ, expected_typ with
    ...
    | TVar _, TVar _ ->
      ...
      if (equal_typ t1 current_typ) then update_substitution sst expected_typ t1
    ...
    | _, _ -> error ("unify_typ: found typ: " ^ (pr_type current_typ) ^
                     " but expect typ: " ^ (pr_type expected_typ))
...
and annotate_typ_exps prog env es : exp list * environment =
  let es, env = List.fold_left ~f:(fun (aes, env) e ->
    let e, env = annotate_typ_exp prog env e in
    (aes @ [e], env)) ~init:([], env) es in
  (es, env)
...
let rec annotate_typ_formula prog env f : formula * environment =
  ...
  | Data (e, t, es, addr, l) ->
    let e, env = annotate_typ_exp prog env e in
    let es, env = annotate_typ_exps prog env es in
    let addr, env = match addr with
      | None -> None, env
      | Some a ->
        let addr, env = annotate_typ_addr_exp prog env a in
        (Some a, env) in
    let dn = pr_type t in
    let env = match env dn with
      | TUnk ->
...
and annotate_typ_formulas prog env fs : formula list * environment =
  let fs, env = List.fold_left ~f:(fun (afs, env) f ->
    let f, env = annotate_typ_formula prog env f in
    (afs @ [f], env)) ~init:([], env) fs in
  (fs, env)
...
let annotate_typ_entailments prog env ents : entailment list * environment =
  let ents, env = List.fold_left ~f:(fun (aents, env) ent ->
    let ent, env = annotate_typ_entailment prog env ent in
    (aents @ [ent], env)) ~init:([], env) ents in
  (ents, env)
...
let rec find_substitution_exp env sst expected_typ e : substitution =
  try (
    ...
    | BinExp (Add, e1, e2, l) | BinExp (Sub, e1, e2, l)
    | BinExp (Mul, e1, e2, l) | BinExp (Div, e1, e2, l) ->
      let sst = find_substitution_exp env sst (TInt 32) e1 in
      find_substitution_exp env sst (TInt 32) e2
    | Func (fname, es, t, l) -> (match env fname with
...
let rec find_substitution_form env sst f : substitution =
  try
    ...
    | BinRel (Le, e1, e2, _) | BinRel (Lt, e1, e2, _)
    | BinRel (Ge, e1, e2, _) | BinRel (Gt, e1, e2, _) ->
      let sst = find_substitution_exp env sst (TInt 32) e1 in
      find_substitution_exp env sst (TInt 32) e2
    ...
    | Data (e, t, es, _, _) ->
      let sn = pr_type t in
      let res = match env sn with
        | TFunc (typs, _) ->
          List.fold2_exn ~f:(find_substitution_exp env) ~init:sst typs (e::es)
        | t -> error ("find_subst_data_form " ^ (pr_formula f) ^
                      ": expect TFunc" ^ (pr_type t)) in
      res
    | Pred (pn, es, _) -> (match env pn with
      | TFunc (typs, _) ->
        List.fold2_exn ~f:(find_substitution_exp env) ~init:sst typs es
      | t -> error ("find_subst_pred_form " ^ (pr_formula f) ^
    ...
    | Conj (f1, f2) | Disj (f1, f2) | Star (f1, f2)
    | Wand (f1, f2) | Septract (f1, f2) | Update (f1, f2) ->
      let sst = find_substitution_form env sst f1 in
      find_substitution_form env sst f2
    | Neg g | Forall (_, g) | Exists (_, g) ->
      find_substitution_form env sst g
...
let substitute_vars sst vs : var list =
  List.map ~f:(substitute_var sst) vs
...
let rec substitute_form sst f : formula =
  ...
  | Septract (f1, f2) -> Septract (substitute_form sst f1, substitute_form sst f2)
...
let substitute_entail sst ent : entailment =
  let lhs, rhs = ent.ent_lhs, ent.ent_rhs in
  {ent with ent_lhs = substitute_form sst lhs;
...
let substitute_command sst cmd : command =
  match cmd with
  | CheckSat (f, l) ->
    CheckSat (substitute_form sst f, l)
  | ProveEntails (ents, l) ->
    ProveEntails (substitute_entails sst ents, l)
  | InferFrame (ent, l) ->
    InferFrame (substitute_entail sst ent, l)
...
let substitute_program prog sst : program =
  ...
  {prog with prog_func_defns = fdefns;
...
let infer_typ_data_defn prog env sst ddefn : data_defn * substitution *
                                             environment =
...
let infer_typ_func_defn prog env sst fdefn : func_defn * substitution *
                                             environment =
  let fdefn, sst = match fdefn.funcd_body with
    | None ->
  ...
  let ftyp =
    ...
    let ret_type = match fdefn.funcd_body with
      | None -> fresh_tvar ()
...
let infer_typ_pred_defn prog env sst pdefn : pred_defn * substitution * environment =
  let pdefn, sst =
...
let infer_typ_program prog : program =
  ...
  let ddefns, sst, env = List.fold_left ~f:(fun (dds, sst, env) dd ->
    let dd, sst, env = infer_typ_data_defn prog env sst dd in
    (dds @ [dd], sst, env)) ~init:([], sst, env) prog.prog_data_defns in
  let fdefns, sst, env = List.fold_left ~f:(fun (fds, sst, env) fd ->
    let fd, sst, env = infer_typ_func_defn prog env sst fd in
    (fds @ [fd], sst, env)) ~init:([], sst, env) prog.prog_func_defns in
  let pdefns, sst, env = List.fold_left ~f:(fun (pds, sst, env) pd ->
    let pd, sst, env = infer_typ_pred_defn prog env sst pd in
    (pds @ [pd], sst, env)) ~init:([], sst, env) prog.prog_pred_defns in
  let cmds, sst, env = List.fold_left ~f:(fun (cmds, sst, env) cmd ->
    let cmd, sst, env = infer_typ_command prog env sst cmd in
    (cmds @ [cmd], sst, env)) ~init:([], sst, env) prog.prog_commands in
  let prog = {prog with prog_data_defns = ddefns;
                        prog_func_defns = fdefns;
                        prog_pred_defns = pdefns;
                        prog_commands = cmds} in
