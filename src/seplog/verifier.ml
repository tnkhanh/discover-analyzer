...
let enable_vrs_interact vstate =
  vstate.vrs_interact <- true
...
let mk_star_program_states ?(dfs=[]) ?(vfs=[]) ?(pfs=[]) ?(afs=[]) pstates =
  List.map ~f:(fun ps ->
    let f = mk_f_star_with ps.pgs_formula ~dfs ~vfs ~pfs ~afs in
    {ps with pgs_formula = f}) pstates

let find_host_alloca_pointers pstate ptrs =
  let rec collect_host f = match f with
    | Emp | Pure _ | View _ | Iter _ | Array _ -> []
    | Data df ->
      if List.exists ~f:(fun v -> mem_exp v ptrs) df.data_args then
        [df.data_root]
      else []
    | Star fs ->
      List.fold ~f:(fun acc f -> acc @ (collect_host f))~init:[] fs
...
let pr_buggy_exps prog exps : string =
  ...
  | true ->
    let source_exps =
      exps |> List.fold ~f:(fun acc e ->
        match e with
        | Var v ->
          (match Hashtbl.find prog.LI.prog_llvalue_original_name (pr_var v) with
           | None -> acc
           | Some v -> acc @ [v])
        | _ -> acc) ~init:[] |>
...
let report_bug prog bug exps (instr: LI.instr): bool =
  ...
  let msg =
    "#########################################################\n" ^
    " DETECTING A BUG: " ^ bug ^ "\n" ^
    "   on the pointers: " ^ (pr_buggy_exps prog exps) ^ "\n" ^
    location ^
    "#########################################################\n" in
  let _ = print_endline ("\n" ^ msg) in
  true
...
let check_memory_leak vstate (instr: LI.instr) pstates : bool =
  let rec collect_heap_pointer f = match f with
    | Emp | Pure _ -> []
  ...
  let buggy_exps = List.fold ~f:(fun acc ps ->
    let heap_ptrs = collect_heap_pointer ps.pgs_formula in
    let leaked_ptrs = List.fold ~f:(fun acc2 v ->
      acc2 @ (find_source_pointers ps [v])) ~init:[] heap_ptrs in
    acc @ leaked_ptrs) ~init:[] pstates in
  if buggy_exps != [] then report_bug prog "MEMORY LEAK" buggy_exps instr
  else false

let check_use_after_free vstate instr pstate ptr : bool =
  let prog = vstate.vrs_llvm_prog in
  let parent_ptrs =
    [ptr] |> find_source_pointers pstate |>
    List.filter ~f:(fun e -> mem_exp e pstate.pgs_freed_pointers) in
  if List.is_empty parent_ptrs then false
...
let check_double_free vstate instr pstate func args : bool =
  ...
  let parent_ptrs =
    freed_ptrs |> find_source_pointers pstate |>
    List.filter ~f:(fun e -> mem_exp e pstate.pgs_freed_pointers) in
  if not (LI.is_func_free func) then false
  else if LI.is_func_free func && List.is_empty parent_ptrs then false
...
let find_lib_proc pname : proc_defn option =
  let procs = List.filter ~f:(fun p ->
    String.equal p.procd_name pname) !lib_core.prog_proc_defns in
  match procs with
  | [] -> None
  | [p] -> Some p
...
let encode_block_pre_state blk : view_form =
  ...
  let llargs =
    if LI.is_entry_block_of_function blk then
...
let encode_precond_llproc (func: LI.func) : view_form =
  let args = func |> LI.func_params |>
             List.map ~f:translate_param in
  let vname = prefix_precond ^ (LI.func_name func) in
  mk_view vname args

let encode_postcond_llproc (func: LI.func) : view_form =
  let args = func |> LI.formal_params_of_func |>
             List.map ~f:translate_param in
  let return_typ = func |> LI.llvalue_of_func |> LL.type_of |>
                   LL.return_type |> translate_lltyp in
  let returns = match return_typ with
    | TVoid -> []
...
let get_prepost_llproc (func: LI.func) (args: exp list) (res: exp) =
  ...
  | None ->
    ...
    let postcond =
      let all_args = match typ_of_exp res with
        | TVoid -> args
        | _ -> args @ [res] in
  ...
  | Some pd ->
    ...
    | [sp] -> (substitute_formula sst sp.psp_precond,
...
let analyze_instr_br vstate pstates (instr: LI.instr) =
  ...
  | Some (`Conditional (cond_llval, blk1, blk2)) ->
    ...
    List.fold ~f:(fun acc s ->
      ...
      acc @ [s1; s2]) ~init:[] pstates

(*** instruction unreachable ***)

let analyze_instr_unreachable vstate pstates (instr: LI.instr) =
  let ents = List.map ~f:(fun ps ->
    mk_entailment ps.pgs_formula (mk_f_false ())) pstates in
  List.map ~f:(fun ps ->
    let ent = mk_entailment ps.pgs_formula (mk_f_false ()) in
    {ps with pgs_formula = mk_f_false ();
             pgs_next_block = None;
             pgs_reachable = false;
             pgs_entails = ps.pgs_entails @ [ent]}) pstates
...
let analyze_instr_alloca vstate pstates (instr: LI.instr) =
  ...
  | LL.TypeKind.Struct ->
    ...
    let root = translate_instr instr in
    let elem_typs = alloc_lltyp |> LL.struct_element_types |>
                    Array.to_list |> List.map ~f:translate_lltyp in
    let init_elems = List.map ~f:(fun typ ->
      mk_exp_var (fresh_new_var typ)) elem_typs in
...
let process_one_state_inst_load vstate instr src dst pstate =
  ...
  | True, [frame] ->
    ...
    let eqs = nf |> collect_eq_exp_f |> List.filter ~f:(function
...
let process_getelemptr_array vstate instr pstate src index field dst =
  ...
  let src_array, dst_addr =
    ...
    | [a] ->
      ...
      (src_array, dst_addr)
    | _ -> herror "process_getelemptr: too many array cells of:" pr_exp src in
...
let analyze_instr_icmp vstate pstates (instr: LI.instr) =
  let exp0 = translate_llvalue (LI.operand instr 0) in
  let exp1 = translate_llvalue (LI.operand instr 1) in
  let cmp = match LI.predicate_of_instr_icmp instr with
...
let analyze_instr_fcmp vstate pstates (instr: LI.instr) =
  let exp0 = translate_llvalue (LI.operand instr 0) in
  let exp1 = translate_llvalue (LI.operand instr 1) in
  let cmp = match LI.predicate_of_instr_fcmp instr with
...
let process_one_state_instr_call vstate instr pstate func args return =
  ...
  | _ ->
    ...
    if buggy then {pstate with pgs_is_buggy = true}
    else pstate
...
let analyze_instr_call vstate pstates (instr: LI.instr) =
  ...
  | false ->
    ...
    List.map ~f:(fun pstate ->
      process_one_state_instr_call vstate instr pstate func args return) pstates
...
let analyze_instruction vstate pstates (instr: LI.instr) =
  ...
  let pstates = match op with
    ...
    | OC.Add | OC.FAdd | OC.Sub | OC.FSub
    | OC.Mul | OC.FMul | OC.UDiv | OC.SDiv | OC.FDiv ->
      analyze_instr_arith vstate pstates instr
...
let symexec_block vstate (blk: LI.block) : entailment list =
  ...
  let init_entails = match predecessors with
    | [] ->
  ...
  let post_states = LI.fold_left_instrs ~f:(fun st instr ->
    let _ = print_endline ("\n" ^ (pr_program_states st) ^ "\n") in
    let _ = print_endline ("++" ^ (LI.pr_instr instr)) in
    analyze_instruction vstate st instr) ~init:init_states blk in
  let _ = print_endline ("\n" ^ (pr_program_states post_states)) in
  let final_states = List.map ~f:(fun ps ->
    let hbody = ps.pgs_formula in
    let hhead = match ps.pgs_next_block with
    ...
    propagate_program_state_entails ps [ent]) post_states in
  let all_entails = List.fold ~f:(fun acc s ->
    acc @ s.pgs_entails) ~init:[] final_states in
  let _ = debugc ("--------\nEntailments: " ^ (LI.block_name blk) ^ ": \n" ^
                  (pr_ents all_entails) ^ "\n") in
  all_entails

let symexc_function vstate (func: LI.func) : entailment list =
  if List.is_empty (LI.blocks_of_func func) then []
  else (
    hdebugc "Analyzing function: " LI.func_name func;
    LI.fold_left_blocks ~f:(fun acc blk ->
      acc @ (symexec_block vstate blk)) ~init:[] func)

let symexec_program vstate (prog: LI.program) : entailment list =
  (* user functions *)
  let ents = List.fold_left ~f:(fun acc f ->
    acc @ (symexc_function vstate f)) ~init:[] prog.prog_user_funcs in
  debugc ("=======================================\n" ^
             "ALL ENTAILMENTS:\n" ^ (pr_ents ents));
