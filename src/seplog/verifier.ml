    ...
    let source_exps =
      ...
      |> List.fold
           ~f:(fun acc e ->
             ...
             | Var v ->
               (match
                  Hashtbl.find prog.prog_meta_data.pmd_llvalue_original_name
                    (pr_var v)
                with
               | None -> acc
               | Some v -> acc @ [ v ])
...
let encode_postcond_llproc (func : LI.func) : view_form =
  ...
  let return_typ =
    func |> LI.llvalue_of_func |> LL.type_of |> LL.return_type
    |> translate_lltyp in
  let returns =
...
let analyze_instr_arith vstate pstates (instr : LI.instr) =
  ...
  let src =
    ...
    | _ -> errorh "analyze_instr_arith: unhandle opcode" LI.pr_opcode op in
  let dst = translate_instr instr in
  let pf = mk_eq dst src in
  mk_star_program_states pstates ~pfs:[ pf ]
;;
...
let analyze_instr_alloca vstate pstates (instr : LI.instr) =
  ...
  | LL.TypeKind.Struct ->
    ...
    let root = translate_instr instr in
    let elem_typs =
      alloc_lltyp |> LL.struct_element_types |> Array.to_list
      |> List.map ~f:translate_lltyp in
    let init_elems =
      List.map ~f:(fun typ -> mk_exp_var (fresh_new_var typ)) elem_typs in
    let _ = debugh "Struct Typ: " pr_type struct_typ in
    let _ = debugh "Root Typ: " pr_type (typ_of_exp root) in
...
let process_one_state_inst_load vstate instr src dst pstate =
  ...
  let _ = debugh "LOAD: Prove Pre-Condition By Frame:\n\n" pr_ent ent in
  let res, frames = PV.infer_entailment_frame vstate.vrs_core_prog ent in
  let _ = debugh "==> Result: " pr_bool_option res in
  let _ = debugh "==> Frame: " pr_fs frames in
...
let process_getelemptr_array vstate instr pstate src index field dst =
  ...
  let src_array, dst_addr =
    ...
    match addrs with
    | [] -> errorh "process_getelemptr: array root not found:" pr_exp src
    | [ a ] ->
      ...
      src_array, dst_addr
    | _ -> errorh "process_getelemptr: too many array cells of:" pr_exp src
  in
  let ent =
    let rhs = mk_f_exists src_evs (mk_f_array src_array) in
    mk_entailment pstate.pgs_formula rhs in
  (* let _ = ndebughc "GetElemPtr: Entailment:\n" pr_ent ent in *)
  let res, frames = PV.infer_entailment_frame vstate.vrs_core_prog ent in
  match res, frames with
  | Some true, [ frame ] ->
...
let process_getelemptr_struct vstate instr pstate src index field dst =
  ...
  let src_array, dst_addr =
    ...
    match addrs with
    | [] ->
      errorh "process_getelemptr_struct: struct root not found:" pr_exp src
    | [ a ] ->
      let src_size = src_size |> mk_add a.addr_elem |> NO.simplify_arith_e in
      let src_array = mk_array a.addr_base src_size src_etyp in
      let dst_index = index |> mk_add a.addr_elem |> NO.simplify_arith_e in
    ...
    | _ -> errorh "process_getelemptr: too many array cells of:" pr_exp src
  in
  (* let _ = ndebughc "Src array: " pr_array_form src_array in *)
  let ent =
    let rhs = mk_f_exists src_evs (mk_f_array src_array) in
    mk_entailment pstate.pgs_formula rhs in
  (* let _ = ndebughc "GetElemPtr: Entailment:\n" pr_ent ent in *)
  let res, frames = PV.infer_entailment_frame vstate.vrs_core_prog ent in
  (* let _ = ndebughc "==> Result: " pr_ternary res in *)
  (* let _ = ndebughc "==> Frame: " pr_fs frames in *)
  match res, frames with
  | Some true, [ frame ] ->
    let npstate = { pstate with pgs_formula = frame } in
    if check_buffer_overflow vstate instr npstate src src_size index
...
let analyze_instr_getelemptr vstate pstates (instr : LI.instr) =
  let src = translate_llvalue (LI.operand instr 0) in
  let _ = debugh "SRC:" pr_exp src in
  let elem = translate_llvalue (LI.operand instr 1) in
  let field = translate_llvalue (LI.operand instr 2) in
  let _ = debugh "GetElemPtr ELEM: " pr_exp elem in
  let _ = debugh "           FIELD: " pr_exp field in
  let dst = translate_instr instr in
  List.map
    ~f:(fun pstate ->
      let src_typ = typ_of_exp src in
      let _ = debugh "SRC_TYP: " pr_type src_typ in
      match src_typ with
      | TPointer (TStruct _) ->
...
let process_one_state_instr_bitcast vstate pstate instr src dst =
   ...
   * let _ = debugh ~compact:true "Bitcast Entailment:\n" pr_ent ent in
   * (\* TRUNG: possibly no need to check entailment for BitCast. *\)
   * let res, frames = PV.infer_entailment_frame prog ent in
   * let _ = debugh ~compact:true "==> Result: " pr_ternary res in
   * let _ = debugh ~compact:true "==> Frame: " pr_fs frames in
   * match res, frames with
   * | True, [frame] ->
   ...
   *     let _ = debugh ~compact:true "SRC_MEMTYP: " pr_type src_memtyp in
   *     let _ = debugh ~compact:true "DST_LLETYP: " pr_type dst_lletyp in
   *     let size_pf =
   *       let src_memtyp_size = mk_exp_int (Int64.to_int_exn src_memtyp_size) in
   *       let dst_etyp_size = Int64.to_int_exn dst_lletyp_size in
   *       let _ = debugh ~compact:true "src_memtyp_size: " pr_exp src_memtyp_size in
   *       let _ = debugh ~compact:true "dst_lletyp_size: " pr_int dst_etyp_size in
   *       let dst_size = mk_mul_exp_int dst_asize dst_etyp_size in
   *       let _ = debugh ~compact:true "dst_size: " pr_exp dst_size in
...
let process_one_state_instr_call vstate instr pstate func args return =
  ...
  let _ = debugh "CALL: Prove Pre-Condition By Frame:\n\n" pr_ent ent in
  let res, frames = PV.infer_entailment_frame vstate.vrs_core_prog ent in
  let _ = debugh "==> Result: " pr_bool_option res in
  let _ = debugh "==> Frame: " pr_fs frames in
  match res, frames with
...
let analyze_instruction vstate pstates (instr : LI.instr) =
  ...
  let pstates =
    ...
    | _ -> errorh "analyze_instruction: need to handle" LI.pr_instr instr in
...
let symexec_block vstate (blk : LI.block) : entailment list =
  let prog = vstate.vrs_llvm_prog in
  debug ("Analyzing Block: " ^ LI.block_name blk ^ ":\n" ^ LI.pr_block blk);
...
let symexc_function vstate (func : LI.func) : entailment list =
  ...
  then []
    ...
    debugh ~marker:false "Analyzing function: " LI.func_name func;
    LI.fold_left_blocks
...
let symexec_program vstate (prog : LI.program) : entailment list =
  ...
  let _ = debugh ~header:true "ALL ENTAILMENTS:" pr_ents ents in
