...
type func_symbol =
  | Add | Sub | Mul | Div               (* binary functions *)
  | FName of string
[@@deriving equal]

type func_exp = {
  func_name : func_symbol;
}
...
type relation_symbol =
  | Eq | Ne | Lt | Gt | Le | Ge         (* binary relations *)
  | RName of string
...
type iter_form = {          (* iterated separating conjunction *)
  iter_base : exp;
  iter_element_index : exp;
  iter_running_index : var;
  iter_begin : exp;
  iter_end : exp;
  (* iter_content : ?: *) (* TODO: how to model content? *)
}
...
type array_form = {
  ...
  array_size : exp;
  array_typ : typ;
  array_update : data_form list;      (* update is always applied first *)
...
let rec pr_exp (e: exp) : string = match e with
  | Void -> "void"
  | Null -> "null"
  | Int i -> pr_int i
  | Float f -> pr_float f
  | String s -> "\"" ^ s ^ "\""
  | Var v -> pr_var v
  | Cast (e, t1, t2) -> "cast(" ^ (pr_exp e) ^ "," ^ (pr_type t1) ^
                        "," ^ (pr_type t2) ^ ")"
...
let pprint_form (d: data_form) : string =
  let addr = match d.data_addr with
    | None -> ""
    | Some addr -> "@" ^ (pr_addr_form addr) in
  (pr_exp d.data_root) ^ "->" ^ (pr_type d.data_typ) ^
  "{" ^ (pr_exps d.data_args) ^ "}" ^ addr

let pr_df (s: data_form) =
  pprint_form s

let pr_dfs sfs : string =
  pr_list ~obrace:"[" ~cbrace:"]" ~sep:", " pr_df sfs

let pr_view_form (v: view_form) : string =
  v.view_name ^ "(" ^ (pr_exps v.view_args) ^ ")"

let pr_vf (v: view_form) =
  pr_view_form v

let pr_vfs vfs : string =
  pr_list ~obrace:"[" ~cbrace:"]" ~sep:", " pr_vf vfs

let pr_iter_form (i: iter_form) =
  "aiter(" ^ (pr_exp i.iter_base) ^ "," ^ (pr_exp i.iter_element_index) ^ ")" ^
  "{" ^ (pr_var i.iter_running_index) ^ "," ^ (pr_exp i.iter_begin) ^ "," ^
  (pr_exp i.iter_end) ^ "}"

let pr_if (i: iter_form) =
  pr_iter_form i

let pr_array_form (a: array_form) =
  let res = "array(" ^ (pr_exp a.array_root) ^ "," ^
               (pr_exp a.array_size) ^ "," ^ (pr_type a.array_typ) ^ ")" in
  let res = List.fold_left ~f:(fun acc d ->
    "(" ^ acc ^ " *+ " ^ (pr_df d) ^ ")") ~init:res a.array_update in
  let res = List.fold_left ~f:(fun acc d ->
    "(" ^ acc ^ " *- " ^ (pr_df d) ^ ")") ~init:res a.array_subtract in
  res

let pr_af (a: array_form) =
  pr_array_form a

let pr_formula (r: formula) : string =
  let rec pr_core r = match r with
    | Emp -> "emp"
    | Pure p -> pr_pf p
    | Data s -> pr_df s
    | View v -> pr_vf v
...
let pr_func_defn (f: func_defn) : string =
  let header = "func " ^ f.funcd_name ^
               "(" ^ (pr_vars f.funcd_params) ^ ") := " in
  let body = match f.funcd_body with
    | None -> "?"
    | Some e -> pr_exp e in
  header ^ body ^ ";"

let pr_reln_defn (r: reln_defn) : string =
  let header = "rel " ^ r.relnd_name ^
               "(" ^ (pr_vars r.relnd_params) ^ ") := " in
  let body = match r.relnd_body with
    | None -> "?"
    | Some f -> pr_pf f in
  header ^ body ^ ";"

let pr_data_defn (d: data_defn) : string =
  let fields = d.datad_fields |>
...
let pr_view_defn (v: view_defn) : string =
  let header = "view " ^ v.viewd_name ^
               "(" ^ (pr_vars v.viewd_params) ^ ") := " in
  let body = match v.viewd_body with
    | [] -> "?"
    | fs -> pr_list ~sep:"\n    \\/ " pr_view_defn_case fs in
  header ^ body ^ ";"

let pr_command (c: command) = match c with
...
let equal_df d1 d2 =
  String.equal (pr_df d1) (pr_df d2)

let equal_vf v1 v2 =
  String.equal (pr_vf v1) (pr_vf v2)
...
let equal_df_form_name_root f1 f2 =
  match f1, f2 with
  | Data d1, Data d2 ->
    (equal_typ d1.data_typ d2.data_typ) &&
    (equal_exp d1.data_root d2.data_root)
  | _ -> false
...
let compare_f f1 f2 =
  let encode f = match f with
    | Pure _ -> 0 | Emp -> 1
    | Data _ -> 2 | View _ -> 3 | Array _ -> 4
    | _ -> 5 in
  let cmp = (encode f1) - (encode f2) in
  if cmp != 0 then cmp
  else String.compare (pr_f f1) (pr_f f2)

(*******************************************************************
 ** type queries
 *******************************************************************)

let typ_of_exp e = match e with
  | Void _ -> TVoid
...
let get_pointer_elem_typ t = match t with
...
let fv_es (es: exps) : var list =
  es |> List.map ~f:fv_e |> merge_vs

let fv_reln_form (rf: reln_form) =
  fv_es (snd rf)
...
let fv_pfs (ps: pure_forms) : var list =
  ps |> List.map ~f:fv_pf |> merge_vs

let fv_data_form (d: data_form) : var list =
  fv_es (d.data_root::d.data_args)
...
let fv_view_form (v: view_form) : var list =
  fv_es v.view_args
...
let fv_fs (fs: formulas) : var list =
  fs |> List.map ~f:fv_f |> merge_vs
...
let is_int_exp e = match typ_of_exp e with
  | TInt _ -> true
  | _ -> false

let is_expr_null e = match e with
  | Null -> true
  | _ -> false

let is_exp_var e = match e with
  | Var v -> true
  | _ -> false

let is_pf_true p = match p with
  | Bool true -> true
  | _ -> false

let is_pf_false p = match p with
  | Bool false -> true
  | _ -> false

let is_pf_exists p = match p with
  | PExists _ -> true
  | _ -> false

let is_pf_forall p = match p with
  | PForall _ -> true
  | _ -> false

let rec is_f_false p = match p with
  | Pure p -> is_pf_false p
  | Star fs -> List.exists ~f:is_f_false fs
  | _ -> false

let is_f_true f = match f with
  | Pure p -> is_pf_true p
  | _ -> false

let is_f_exists f = match f with
  | Exists _ -> true
  | _ -> false

let rec is_f_pure f = match f with
  | Pure _ | Emp _ -> true
...
let is_f_emp f = match f with
  | Emp -> true
  | _ -> false

let is_f_data (f: formula) = match f with
  | Data  _ -> true
  | _ -> false

let is_f_view (f: formula) = match f with
  | View _ -> true
  | _ -> false

let is_f_array (f: formula) = match f with
  | Array _ -> true
  | _ -> false

let is_f_data_or_view (f: formula) = match f with
  | Data _ -> true
  | View _ -> true
  | _ -> false
...
let mk_exp_var_typ vname typ =
  Var (mk_var vname typ)

let fresh_exp_var ?(name="t") typ =
  Var (fresh_new_var ~name:name typ)

let mk_cast_exp e old_typ new_typ =
  Cast (e, old_typ, new_typ)
...
let mk_sub e1 e2 =
  ...
  let typ = match t1, t2 with
    | TFloat, _ | _, TFloat -> TFloat
    | TInt i1, TInt i2 -> TInt (max i1 i2)
    | _ -> error ("mk_sub " ^ (pr_exp e1) ^ " and " ^ (pr_exp e2) ^
...
let mk_mul_exp_int e i =
  mk_mul e (mk_exp_int i)
...
let mk_func ?(typ=TUnk) fn args =
  Func (FName fn, args, typ)
...
let mk_ne_exps exps : pure_form =
  let rec mk_aux es = match es with
    | [] -> []
    | e::es ->
...
let mk_addr_base base =
  mk_addr base (mk_exp_int 0) (mk_exp_int 0)
...
let mk_f_data ?(addr=None) typ root args =
  Data (mk_data ~addr typ root args)

let mk_f_view name args =
  View (mk_view name args)
...
let mk_f_star fs =
  ...
  let fs, ps = List.fold_left ~f:(fun (afs, aps) f ->
    match f with
    | Emp -> (afs, aps)
    | Pure p -> (afs, aps @ [p])
    | _ -> (afs @ [f], aps)) ~init:([], []) fs in
  let fs = match ps with
    | [] -> fs
...
let mk_f_exists vs f =
  let rec make vs f = match vs, f with
    | [], _ -> f
...
let mk_func_defn name params body =
  let return_typ = match body with
    | None -> TUnk
...
let mk_cmd_check_sat f =
  CheckSat f

let mk_cmd_prove_entailments ents =
  ProveEntails ents
...
let pr_equality (eq: equality) : string = match eq with
...
let reset_substitution_vs (sst: substitution) (vs: var list) =
  fun v ->
    if (member_vs v vs) then mk_exp_var v
    else sst v

let init_subst () : substitution =
  fun v -> mk_exp_var v
...
let substitute_pure_form (sst: substitution) (p: pure_form) =
  let rec subst sst p = match p with
    | Bool _ -> p
    ...
    | PExists (vs, f) -> PExists (vs, subst (reset_substitution_vs sst vs) f) in
  subst sst p

let subst_data_form (sst: substitution) (d: data_form) =
  {d with data_root = substitute_exp sst d.data_root;
...
let substitute_iter_form (sst: substitution) (i: iter_form) =
  {i with iter_base = substitute_exp sst i.iter_base;
...
let substitute_formula (sst: substitution) (f: formula) : formula =
  let rec subst sst f = match f with
    ...
    | Exists (vs, f) -> Exists (vs, subst (reset_substitution_vs sst vs) f) in
  subst sst f

let get_subst_of_vars vars (eqs: equalities) : substitution =
  List.fold_left ~f:(fun acc eq ->
    match eq with
    | EqExp (v, e) when List.exists ~f:(equal_var v) vars ->
      if List.is_empty (intersect_vs (fv_e e) vars) then
        extend_subst acc v e
      else acc
    | _ -> acc) ~init:(init_subst ()) eqs

let get_substitute_formula_eqs (eqs: equalities) : substitution =
  let sst, _ = List.fold_left ~f:(fun (sst, used_vars) eq ->
    match eq with
...
let mk_renaming_fresh (vs: var list) : renaming =
  List.fold_left ~f:(fun acc v ->
    extend_renaming acc v (fresh_old_var v)) ~init:(init_renaming ()) vs

let mk_renaming_of_vars (vs1: vars) (vs2: vars): renaming =
  if List.length vs1 != List.length vs2 then
    error "mk_renaming_of_vars: different length"
  else List.fold2_exn ~f:(fun acc v1 v2 ->
    extend_renaming acc v1 v2) ~init:(init_renaming ()) vs1 vs2

let rename_var (rnm: renaming) (v: var) : var =
  rnm v

let rename_v =
  rename_var

let rename_vs (rnm: renaming) (vs: var list) : var list =
  List.map ~f:rnm vs

let rename_exp (rnm: renaming) (e: exp) : exp =
  let rec rename e = match e with
    | Void _ | Null _ | Int _ | Float _ | String _ -> e
    | Var v -> Var (rnm v)
...
let rename_data_form (rnm: renaming) (d: data_form) =
  {d with data_root = rename_e rnm d.data_root;
...
let rename_iter_form (rnm: renaming) (i: iter_form) =
  ...
  {i with iter_base = rename_e rnm i.iter_base;
...
let rename_array_form (rnm: renaming) (a: array_form) =
  {a with array_root = rename_e rnm a.array_root;
...
let rename_form (rnm: renaming) (f: formula) =
  let rec rename rnm f = match f with
    | Emp -> f
...
let rec translate_lltyp (typ: LL.lltype) : typ =
  ...
  | LL.TypeKind.X86fp80
  | LL.TypeKind.Fp128
  | LL.TypeKind.Ppc_fp128 ->
    TFloat
  | LL.TypeKind.Integer ->
    TInt (LL.integer_bitwidth typ)
  | LL.TypeKind.Pointer ->
    TPointer (translate_lltyp (LL.element_type typ))
  | LL.TypeKind.Array ->
    TArray
  | LL.TypeKind.Struct ->
    let _ = debugc "TRANSLATE TYPE STRUCT" in
    TStruct (Option.value (LL.struct_name typ) ~default:"unknown")
  | LL.TypeKind.Function
  | LL.TypeKind.Vector
  | LL.TypeKind.X86_mmx -> TUnk
...
let translate_llvalue (v: LL.llvalue) : exp =
  ...
  if LI.is_llvalue_undef v then mk_null ()
  else (
    let vname = LL.value_name v in
    let res = match typ with
      | TInt _ -> (match LI.int_of_const v with
        | None -> mk_exp_var_typ vname typ
        | Some i -> mk_exp_int i)
      | TFloat -> (match LL.float_of_const v with
        | None -> mk_exp_var_typ (LL.value_name v) typ
        | Some i -> mk_exp_float i)
      | TString -> (match LL.string_of_const v with
        | None -> mk_exp_var_typ (LL.value_name v) typ
...
let extract_pure_form (f: formula) : pure_form =
  let rec extract f = match f with
    | Pure p -> p
    | Emp -> mk_pf_true ()
    | Data _ | View _ | Iter _ | Array _
    | Star _ | Wand _ | Septract _ ->
...
let mk_f_star_with ?(dfs=[]) ?(vfs=[]) ?(afs=[]) ?(pfs=[]) ?(fs=[]) f =
  ...
  let nfs = (List.map ~f:f_of_data dfs) @
            (List.map ~f:f_of_view vfs) @
            (List.map ~f:f_of_array afs) @
            (List.map ~f:f_of_pure pfs) @ fs in
  mk_f_star (f::nfs)
...
let mk_eq_exps_pair (es1: exps) (es2: exps) : pure_form =
  try
    let pfs = List.fold2_exn ~f:(fun acc e1 e2 ->
      acc @ [mk_eq e1 e2]) ~init:[] es1 es2 in
    mk_pconj pfs
...
let collect_func_name_p (p: pure_form) : string SP.t =
  let rec get_func f = match f with
    | Reln (_, es) ->
      SP.union_list (List.map ~f:collect_func_name_e es)
    | PNeg f | PForall (_, f) | PExists (_, f) -> get_func f
    | PConj fs | PDisj fs ->
      SP.union_list (List.map ~f:get_func fs)
    | _ -> SP.empty in
  get_func p
...
let collect_reln_name_p (p: pure_form) : string SP.t =
  let rec get_rel f = match f with
    | Reln (RName rn, es) ->
      SP.singleton rn
    | PNeg f | PForall (_, f) | PExists (_, f) ->
      get_rel f
    | PConj fs | PDisj fs ->
      SP.union_list (List.map ~f:get_rel fs)
    | _ -> SP.empty in
  get_rel p
...
let find_data_defn ddefns (dname: string) : data_defn option  =
  List.find ~f:(fun d ->
    String.equal (pr_type d.datad_typ) dname) ddefns
...
let unfold_view_form vdefns (vf: view_form) : view_defn_case list =
  ...
  | Some vd ->
    ...
    List.map ~f:(fun vdc ->
      ...
      {vdc with vdc_form = f}) vd.viewd_body
...
let replace_reln_form_p (head: reln_form) (body: pure_form) p : pure_form =
  let rec replace p = match p, head with
    | Bool _, _ | BExp _, _ -> p
    | Reln (RName rnp, esp), (RName rnh, esh) when String.equal rnp rnh ->
      let args, pfs = List.fold_left ~f:(fun (aargs, apfs) e ->
        match e with
        | Var v -> (aargs @ [v], apfs)
        | _ ->
          let v = fresh_new_var (typ_of_exp e) in
          (aargs @ [v], apfs @ [mk_eq (mk_exp_var v) e])) ~init:([],[]) esh in
      let body =
...
let replace_reln_form_f (head: reln_form) (body: pure_form) f : formula =
  let rec replace f = match f with
...
let replace_view_form_f (head: view_form) (body: formula) f : formula =
  let rec replace f = match f with
    ...
    | View vf ->
      if String.equal vf.view_name head.view_name then
        let args, pfs = List.fold_left ~f:(fun (aargs, apfs) e ->
          match e with
          | Var v -> (aargs @ [v], apfs)
          | _ ->
...
let find_addr_exp_of_exp (f: formula) (e: exp) : addr_exp list =
  let rec find_root g = match g with
    | Emp | Pure _ | View _ -> []
    | Data d ->
      if equal_exp d.data_root e then [mk_addr_base d.data_root]
      else []
    | Array a ->
      if equal_exp a.array_root e then [mk_addr_base a.array_root]
      else (
        let ds = List.find ~f:(fun d -> equal_exp d.data_root e) a.array_update in
        match ds with
        | None -> []
        | Some d -> match d.data_addr with
          | None -> []
          | Some a -> [a])
    | Star gs ->
      List.fold_left ~f:(fun acc g -> acc @ (find_root g)) ~init:[] gs
