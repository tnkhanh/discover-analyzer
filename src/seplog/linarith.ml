...
let exp_of_term (t: term) : exp =
  let rec convert t = match t with
    ...
    | ((c, v) :: k, i) ->
      ...
      | Some e1 ->
        if (c = 0) then Some e1
        else if (c = 1) then Some (mk_add sv e1)
        else if (c = -1) then Some (mk_sub e1 sv)
        else if (c < 0) then
...
let term_of_exp (e: exp) : term * (exp option) =
  let rec convert e = match e with
    ...
    | Func (Mul, [e1; e2], _) ->
      let ((k1, i1), o1) = convert e1 in
      let ((k2, i2), o2) = convert e2 in
      if (List.is_empty k1) && (Option.is_none o1) then
        ...
        (nt,ne)
      else if (List.is_empty k2) && (Option.is_none o2) then
