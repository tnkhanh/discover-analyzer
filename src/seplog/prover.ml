...
let iprint pstate msg =
  if pstate.prs_interact then debugc msg
  else ()
...
and remove_useless_rules pstate goal rules : rule list =
  List.filter ~f:(fun r ->
    not (is_rule_useless pstate goal rules r)) rules
...
and select_one_candidate_rule pstate goal rules : (rule * rule list) option =
  ...
  | true, _ ->
    ...
    if String.equal decicion "a" then
...
and process_all_rules pstate goal rules : proof_tree =
  let rec process_rules ptrees rules =
    ...
    | true, [] ->
      ...
      if (String.equal decision "b") then Some (mk_proof_tree_unknown goal)
      else None
    | false, [] -> None
    | _ ->
