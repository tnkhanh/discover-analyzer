...
let pr_id (vn : identifier) : string =
  ...
  match index with
  | None -> base
  | Some i -> base ^ (string_of_int i)

let rec pr_type (t: typ) : string = match t with
  | TVoid -> "void"
  | TBool -> "bool"
  | TInt i -> "i" ^ (string_of_int i)
  | TFloat -> "float"
  | TString -> "string"
  | TStruct n -> n
...
let pr_var (v: var) : string =
  let (id, typ) = v in
  if not !print_type then pr_id id
...
let dedup_vs vs =
  let rec dedup xs acc = match xs with
    | [] -> acc
    | x::xs ->
      if member_vs x acc then dedup xs acc
      else dedup xs (x::acc) in
  dedup vs []
...
let is_typ_var t = match t with
  | TVar _ -> true
  | _ -> false

let is_typ_known t = match t with
  | TVar _ -> false
  | _ -> true
