...
let rec typ_of_exp e = match e with
  | Int (_, _) -> TInt 32
  | Float (_, _) -> TFloat
  | String (_, _) -> TString
...
let rec pr_formula (f: formula) : string = match f with
  | Bool (b, _) -> pr_bool b
  | BVar (v, _) -> pr_var v
...
let pr_predicate_defn (p: pred_defn) : string =
  let pred_type = match p.predd_typ with
    | PtView -> "view"
    | PtReln -> "rel" in
  let header = pred_type ^ " " ^ p.predd_name ^
               "(" ^ (pr_vars p.predd_params) ^ ") := " in
  let body = match p.predd_body with
    | [] -> "?"
    | fs -> pr_list ~sep:"\n    \\/ " pr_formula fs in
  header ^ body ^ ";"
...
let pr_proc_defn (p: proc_defn) : string =
  let params = p.procd_params |>
               List.map ~f:(fun v ->
                 (pr_type (typ_of_var v)) ^ " " ^ (pr_var v)) |>
               String.concat ~sep:"," in
  let specs = p.procd_specs |>
              List.map ~f:(fun s ->
                "  requires " ^ (pr_formula s.psp_precond) ^ "\n" ^
                "  ensures " ^ (pr_formula s.psp_postcond) ^ ";") |>
              String.concat ~sep:"\n" in
...
let pr_command (c: command) = match c with
  | CheckSat (f, _) ->
    "CheckSat: " ^ (pr_formula f) ^ ";"
  | ProveEntails (ents, _) ->
    "ProveEntails: \n" ^ (pr_ents ents) ^ ";"
  | InferFrame (ent, _) ->
    "FindFrame: " ^ (pr_ent ent) ^ ";"
...
let find_data_defn (prog: program) (dname: string) : data_defn option  =
  List.find ~f:(fun d ->
    String.equal (pr_type d.datad_typ) dname) prog.prog_data_defns
