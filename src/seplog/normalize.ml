...
let simplify_tauto_contra_p (f: pure_form) : pure_form * bool =
  ...
  let rec simplify p = match p with
    ...
    | Reln (Eq, [a; b]) when equal_exp a b ->
      return_changed (Bool true)
    | Reln (Eq, [Func (Sub, [a; b], _); Int 0]) when equal_exp a b ->
      return_changed (Bool true)
    | Reln (Ne, [a; b]) when equal_exp a b ->
      return_changed (Bool false)
    | Reln _ -> p
    | PNeg g -> (match simplify g with
      | Bool true -> return_changed (Bool false)
      | Bool false -> return_changed (Bool true)
      | ng -> mk_pneg ng)
    | PConj gs ->
      let ngs = gs |> List.map ~f:simplify |>
                List.exclude ~f:is_pf_true in
      if List.length ngs > 1 && List.exists ~f:is_pf_false ngs then
        return_changed (mk_pf_false ())
      else if List.length ngs = List.length gs then mk_pconj ngs
      else return_changed (mk_pconj ngs)
    | PDisj gs ->
      let ngs = gs |> List.map ~f:simplify |>
                List.exclude ~f:is_pf_false in
      if List.length ngs > 1 && List.exists ~f:is_pf_true ngs then
        return_changed (mk_pf_true ())
      else if List.length ngs = List.length gs then mk_pdisj ngs
      else mk_pdisj ngs
...
let simplify_tauto_contra_f (f: formula) : formula * bool =
  ...
  let rec simplify f = match f with
    ...
    | Star fs ->
      let nfs = fs |> List.map ~f:simplify |>
    ...
    | Wand (f1, f2) ->
      ...
      if is_f_emp f1 || is_f_true f1 then return_changed f2
...
let substitute_var_by_equality_p keep_vars f : pure_form =
  let rec simplify kvs p = match p with
    | Bool _ | BExp _ | BEq _ | Reln _ -> p
    ...
    | PConj gs ->
      ...
      let eqs =
        ...
        List.filter ~f:(function
          ...
          | EqPure (_, p) -> subset_vs (fv_pf p) kvs) eqs in
...
let substitute_var_by_equality_f keep_vars f : formula =
  let rec simplify kvs f = match f with
...
let simplify_arith_p (f: pure_form) =
  ...
  let rec simplify g = match g with
    | Bool _ | BExp _ -> g
    | Reln (reln, [e1; e2]) when is_int_exp e1 ->
      simplify_reln reln e1 e2
    | Reln _ -> g
...
let simplify_arith_sf d =
  {d with data_root = simplify_arith_e d.data_root;
          data_args = simplify_arith_es d.data_args;
          data_addr = Option.map ~f:simplify_arith_ad d.data_addr}

let simplify_arith_vf v =
  {v with view_args = simplify_arith_es v.view_args}

let simplify_arith_if i =
  {i with iter_base = simplify_arith_e i.iter_base;
...
let simplify_arith_af a =
  {a with array_root = simplify_arith_e a.array_root;
...
let encode_formula_to_pure (f: formula) : pure_form =
  let rec collect_root f = match f with
    | Emp | View _ | Iter _ | Array _
    | Pure _ | Wand _ | Septract _ -> []
    | Data d -> [d.data_root]
    | Star fs ->
  ...
  let rec encode_sub_form f = match f with
...
let simplify_p (f: pure_form) : pure_form =
  f
