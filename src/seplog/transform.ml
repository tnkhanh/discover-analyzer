...
let rec transform_exp (e: SA.exp) : SI.exp =
  ...
  | SA.Func (fname, es, typ, _) ->
    SI.Func (FName fname, transform_exps es, typ)
...
let rec transform_formula (prog: SA.program) (f: SA.formula) : SI.formula =
  ...
  | SA.BEq (e, f0) ->
    let p = match transform_formula prog f0 with
      | Pure p -> p
  ...
  | SA.Pred (pname, args, _) ->
    ...
    (match SA.find_pred_defn prog pname with
     ...
     | Some pd ->
       if SA.equal_pred_typ pd.SA.predd_typ SA.PtReln then
         SI.mk_f_pure (SI.mk_preln (SI.RName pname) args)
  ...
  | SA.Conj (f1, f2) ->
    let p1, p2 = match transform_formula prog f1, transform_formula prog f2 with
...
let transform_data_defn prog (dd: SA.data_defn) : SI.data_defn =
  {SI.datad_typ = dd.SA.datad_typ;
   SI.datad_fields = dd.SA.datad_fields;}
...
let transform_command prog (cmd: SA.command) : SI.command =
  match cmd with
  | SA.CheckSat (f, _) ->
    SI.CheckSat (transform_formula prog f)
  | SA.ProveEntails (ents, _) ->
    SI.ProveEntails (List.map ~f:(transform_entailment prog) ents)
  | SA.InferFrame (ent, _) ->
    SI.InferFrame (transform_entailment prog ent)
...
let transform_program (prog: SA.program) : SI.program =
  ...
  let relns, views = List.fold_left ~f:(fun (rds, vds) pd ->
    match transform_pred_defn prog pd with
    | PView vd -> (rds, vds @ [vd])
    | PReln rd -> (rds @ [rd], vds)) ~init:([], []) prog.SA.prog_pred_defns in
