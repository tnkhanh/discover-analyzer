...
let rec transform_formula (prog : SA.program) (f : SA.formula) : SI.formula =
  ...
  | SA.BEq (e, f0) ->
    let p =
      ...
      | Pure p -> p
      | _ -> errorp "transform_formula: BEq: not a pure" SA.pr_formula f in
  ...
  | SA.Neg f0 ->
    let p =
      match transform_formula prog f0 with
      | SI.Pure p -> p
      | _ -> errorp "transform_formula: Neg: not a pure" SA.pr_formula f in
    SI.mk_f_pure (SI.mk_pneg p)
  | SA.Conj (f1, f2) ->
    let p1, p2 =
      match transform_formula prog f1, transform_formula prog f2 with
      | SI.Pure p1, SI.Pure p2 -> p1, p2
      | _ -> errorp "transform_formula: Conj: not a pure" SA.pr_formula f in
    SI.mk_f_pure (SI.mk_pconj [ p1; p2 ])
  | SA.Disj (f1, f2) ->
    let p1, p2 =
      match transform_formula prog f1, transform_formula prog f2 with
      | SI.Pure p1, SI.Pure p2 -> p1, p2
      | _ -> errorp "transform_formula: Disj: not a pure" SA.pr_formula f in
  ...
  | SA.Forall (vs, f0) ->
    ...
    | g -> errorp "transform_formula: Forall: expect pure: " SA.pr_formula f)
  | SA.Exists (vs, f0) ->
    (match transform_formula prog f0 with
    | SI.Pure p -> Pure (SI.mk_pforall vs p)
...
let transform_pred_defn prog (pd : SA.pred_defn) =
  ...
  | SA.PtReln, [ f ] ->
    let body =
      match f with
      | SI.Pure p -> Some p
      | _ -> errorp "transform_pred_reln: not a pure body" SI.pr_formula f
    in
  ...
  let relns, views =
    List.fold_left
      ~f:(fun (rds, vds) pd ->
        ...
        | PView vd -> rds, vds @ [ vd ]
        | PReln rd -> rds @ [ rd ], vds)
      ~init:([], []) prog.SA.prog_pred_defns in
