...
let start_solver () =
  ...
  if !proc.PS.proc_pid = PS.pid_dummy then
    let config = [] in
...
let read_output proc : string =
  let rec read acc =
    try
      ...
      if (String.equal line z3eof) then nacc
      else read nacc
...
module Z3SL = struct
  ...
  let transform_bool b =
    if b then "true" else "false"

  let transform_int i =
    string_of_int i

  let transform_float f =
    string_of_float f
  ...
  let transform_typ (t: SI.typ) : string =
    ...
    | TBool -> "Bool"
    | TInt _ -> "Int"
    | _ -> "Int"     (* FIXME: temporarily put as Int type *)
  ...
  and transform_pure_form (f: SI.pure_form) : string =
    ...
    | BEq (e, f) ->
      "(= " ^ (transform_exp e) ^ " " ^ (transform_pure_form f) ^ ")"
    | Reln (Eq, [e1; e2]) ->
      "(= " ^ (transform_exp e1) ^ " " ^ (transform_exp e2) ^ ")"
    | Reln (Ne, [e1; e2]) ->
    ...
    | PNeg f -> "(not " ^ (transform_pure_form f) ^ ")"
    | PConj fs -> "(and " ^ (transform_forms fs) ^ ")"
    | PDisj fs -> "(or " ^ (transform_forms fs) ^ ")"
    | PForall (vs, g) ->
      "(forall (" ^ (transform_vars_with_typ vs) ^ ") " ^ (transform_pure_form g) ^ ")"
    | PExists (vs, g) ->
      "(exists (" ^ (transform_vars_with_typ vs) ^ ") " ^ (transform_pure_form g) ^ ")"
  ...
  let mk_func_defn (fd: SI.func_defn) : string =
    ...
    | Some e ->
      ...
      "(define-fun " ^ fd.funcd_name ^
      " (" ^ param_typs ^ ") " ^ return_typ ^ " " ^ body ^ ")"
  ...
  let mk_reln_defn (pd: SI.reln_defn) : string =
    ...
    | Some f ->
      ...
      "(define-fun " ^ pd.relnd_name ^
      " (" ^ params ^ ") " ^ return_typ ^ " " ^ body ^ ")"

  let mk_assert f =
    "(assert " ^ (transform_pure_form f) ^ ")"

  let mk_input ?(prog=None) ?(mvars=[]) (fs: SI.pure_form list) : string =
    let concat_newlines str =
      if String.equal str "" then str else str ^ "\n\n" in
    let var_decls = fs |> SI.fv_pfs |> (@) mvars |> SI.dedup_vs |>
                    List.map ~f:mk_var_decl |> String.concat ~sep:"\n" in
    let func_decls = match prog with
      | None -> ""
      | Some prog ->
        let fnames = SI.collect_func_name_ps fs in
        let fdefns = SP.fold ~f:(fun acc fn ->
          match SI.find_func_defn prog.SI.prog_func_defns fn with
          | None -> acc
          | Some fd -> SP.add acc (mk_func_defn fd)) ~init:SP.empty fnames in
        String.concat ~sep:"\n" (SP.to_list fdefns) in
    let reln_decls = match prog with
      | None -> ""
      | Some prog ->
        let rnames = SI.collect_reln_name_ps fs in
        let rdefns = SP.fold ~f:(fun acc rn ->
          match SI.find_reln_defn prog.prog_reln_defns rn with
          | None -> acc
          | Some rd -> SP.add acc (mk_reln_defn rd)) ~init:SP.empty rnames in
  ...
  let get_result ?(mvars=[]) z3output : SI.smt_result =
    match z3output with
    | Sat model ->
      let model = List.fold_left ~f:(fun acc (s, i) ->
        try
          let var = List.find_exn ~f:(fun v -> String.equal (SI.pr_var v) s) mvars in
          acc @ [(var, SI.mk_exp_int i)]
        with _ -> acc) ~init:[] (norm_model model) in
      (* sort decreasingly *)
      let model = List.sort
  ...
  let check_sat ?(prog=None) ?(logic="") ?(mvars=[]) fs : SI.smt_result =
    let set_logic =
      if (String.equal logic "") then ""
      else "(set-logic " ^ logic ^ ")\n" in
    let set_option_model, get_model = match mvars with
      | [] -> ("(set-option :produce-models false)\n", "")
      | _ -> ("(set-option :produce-models true)\n", "(get-model)\n") in
    let z3_input =
      set_logic ^
      set_option_model ^ "\n" ^
      (mk_input ~prog:prog ~mvars:mvars fs) ^ "\n\n" ^
      "(check-sat)\n" ^
      get_model in
    ...
    match output with
    | Error msg ->
      let _ = debugc ("Z3SL: error while checking sat:\n" ^
...
module Z3LL = struct
  ...
  let transform_llvalue (v: LI.llvalue) : string =
    ...
    if LL.is_null v then "0"
    else LI.pr_value v

  let transform_predicate (p: LI.predicate) : string =
    let rec transform p = match p with
      | LI.PBool b ->
        if b then "true" else "false"
      | PIcmp (cmp, lhs, rhs) ->
  ...
  let mk_z3_input (p: LI.predicate) : string =
    let vars = p |> LI.collect_llvalue_of_predicate |>
               List.exclude ~f:(fun v ->
                 LI.is_llvalue_null_constant v ||
                 LI.is_llvalue_integer_constant v) in
    let var_decls = vars |> List.map ~f:mk_var_decl |>
                    String.concat ~sep:"\n" in
  ...
  let check_sat (p: LI.predicate) : ternary =
    ...
    match output with
    | Error msg ->
      let _ = debug ("Z3LL: error while checking sat:\n" ^
