...
module SmtSL = struct
  let check_sat ?(prog = None) ?(mvars = []) (fs : SI.pure_forms)
    ...
    let check_sat =
      ...
      | _ -> errorp "check_sat: unknown solver" pr_solver !smt_solver in
    Sys.record_runtime ~f:(fun () -> check_sat fs) time_smt
  ;;
  ...
  let check_imply ?(prog = None) ?(norm = false) f1 f2 : bool option =
    let check_imply =
      ...
      | _ -> errorp "check_imply: unknown solver" pr_solver !smt_solver in
    check_imply f1 f2
  ;;
      ...
      List.map
        ~f:(fun e ->
          let lhs =
            ...
            | false -> errorp "check_sat_horn: not a pure body" SI.pr_ent e
          in
          let rhs =
            match SI.is_f_pure e.SI.ent_rhs with
            | true -> SI.extract_pure_form e.SI.ent_rhs
            | _ -> errorp "check_sat_horn: not a pure horn" SI.pr_ent e in
...
module SmtLL = struct
  let check_sat (p : LI.predicate) : bool option =
    let check_sat =
      ...
      | _ -> errorp "check_sat: unknown solver" pr_solver !smt_solver in
    Sys.record_runtime ~f:(fun () -> check_sat p) time_smt
  ;;
