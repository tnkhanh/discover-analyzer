...
let compile_program (input_file : string) : LI.program =
  ...
  | POutput go_build_output_str ->
    ...
    let _ = debugp "Line starting with WORK=: " pr_str work_line in
    let goc_line =
      try
    ...
    let _ = debugp "gollvm compiled with: " pr_str goc_line in
    ...
    let _ = debugp "replaced $WORK in command: " pr_str fixed_goc_line in
