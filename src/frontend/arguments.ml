...
let rec print_usage () =
  let arguments =
    if !release_mode then arguments_release
    else arguments_raw in
  let welcome_msg =
    if !release_mode then
      ...
      "Usage: " ^ Sys.argv.(0) ^ " [options] <input file>\n\n" in
  let args_msg =
    arguments |> List.fold_left ~f:(fun acc (flags, doc, _) ->
      let opt = Printf.sprintf "  %-20s" (String.concat ~sep:", " flags) in
      let opt =
        if String.length opt > 22 then opt ^ (Printf.sprintf "\n  %-20s" "")
        else opt in
...
and arguments_release = [
  (["--clang-option"], "Command options for Clang",
...
and arguments_raw = [
  ...
  (["--pip"], "Print input programs",
   Arg.Set print_input_prog);

  (["--type"], "Print type information of variables",
   Arg.Set print_type);

  (["--pcp"], "Print core programs",
   Arg.Set print_core_prog);

  (["--pap"], "Print analyzed programs",
   Arg.Set print_analyzed_prog);

  (["--psp"], "Print statistics of programs",
   Arg.Set print_stats_prog);

  (["--pap-off"], "Turn off printing analyzed programs",
   Arg.Clear print_analyzed_prog);

  (["--pco"], "Print concise output",
   Arg.Set print_concise_output);

  (["--pcd"], "Print concise debug",
   Arg.Set print_concise_debug);
  ...
  (["--edf"], "Export debugging information to file",
   Arg.Unit (fun () ->
  ...
  (["--input-horn"], "Proving formulas",
   Arg.Unit (fun () -> input_mode := InpSepLogic));

  (["--input-llvm"], "Analyzing LLVM bitcode",
   Arg.Unit (fun () -> input_mode := InpBitcode));

  (["--clang-option"], "Command options for Clang",
   Arg.Set_string clang_options);

  (["--clang-extra-option"], "Extra command options for Clang",
   Arg.Set_string clang_extra_options);

  (["--opt-option"], "Command options for Opt",
   Arg.Set_string opt_options);

  (*--------------------------------------------------------
   * analysis
   *--------------------------------------------------------*)

  (["--analysis-off"], "Disable all analysis passes",
   Arg.Set skip_analysis);

  (["--symexec"], "Symbolic execution",
   Arg.Unit (fun () -> work_mode := WkmSymExec));

  (["--dataflow"; "--dfa"], "Data Flow Analysis",
   Arg.Unit (fun () -> work_mode := WkmDFA));

  (["--dfa-range"], "Perform the data-flow range analysis",
   Arg.Unit (fun () ->
     work_mode := WkmDFA;
     dfa_sparse_analysis := false;
     dfa_analyses := !dfa_analyses @ [DfaRange]));

  (["--dfa-undef"], "Perform the data-flow undef analysis",
   Arg.Unit (fun () ->
     work_mode := WkmDFA;
     dfa_analyses := !dfa_analyses @ [DfaUndef]));

  (["--dfa-memsize"], "Perform the data-flow memory size analysis",
   Arg.Unit (fun () ->
     work_mode := WkmDFA;
     dfa_analyses := !dfa_analyses @ [DfaMemsize]));

  (["--dfa-pointer"], "Perform the data-flow pointer analysis",
   Arg.Unit (fun () ->
     work_mode := WkmDFA;
     dfa_pointer_conservative := false;
     dfa_analyses := !dfa_analyses @ [DfaPointer]));

  (["--dfa-pointer-conservative"], "Perform the data-flow pointer analysis conservatively",
   Arg.Unit (fun () ->
     work_mode := WkmDFA;
     dfa_pointer_conservative := true;
     dfa_analyses := !dfa_analyses @ [DfaPointer]));

  (["--dfa-pointer-full"], "Perform the data-flow pointer analysis",
   Arg.Unit (fun () ->
     work_mode := WkmDFA;
     dfa_analyses := !dfa_analyses @ [DfaUndef; DfaPointer]));

  (["--dfa-all-analysis"], "Perform all data-flow analyses",
   Arg.Unit (fun () ->
     work_mode := WkmDFA;
  ...
  (["--dfa-intra"], "Intra-procedural analysis",
   Arg.Unit (fun () -> dfa_mode := DfaIntraProc));

  (["--dfa-inter"], "Inter-procedural analysis, from the main function",
  ...
  (["--llvm-source-name"], "Enable LLVM mode_debug with original source info",
   Arg.Set llvm_orig_source_name);

  (["--llvm-prog-info"], "Enable printing program's information",
   Arg.Set llvm_print_prog_info);

  (["--llvm-no-simplify"], "No not simplify LLVM IR",
   Arg.Clear llvm_simplify);

  (["--llvm-no-optimize"], "Do not optimize LLVM IR",
   Arg.Clear llvm_optimize);

  (["--llvm-no-normalize"], "Do not normalize LLVM IR",
   Arg.Clear llvm_normalize);

  (["--llvm-path"], "Path to llvm",
   Arg.Set_string llvm_path);

  (*--------------------------------------------------------
   * gollvm options
   *--------------------------------------------------------*)

  (["--gollvm-path"], "Path to gollvm, e.g., /usr/bin if gollvm is /usr/bin/go",
  ...
  (["--export-entailments"], "Export entailments",
   Arg.Set export_entailment);

  (["--export-bitcode"], "Export LLVM Bitcode",
   Arg.Set export_bitcode);

  (["--ascii"], "Print proof in ascii format",
   Arg.Set export_proof_ascii);
  ...
  (["--debug-function"], "Specify target functions for debugging",
   Arg.String (fun s ->
     mode_debug_function := true;
     regex_debug_function := s));

  (["--debug-working-function"], "Specify target working functions for debugging",
   Arg.String (fun s ->
     mode_debug_working_function := true;
     regex_debug_working_function := s));

  (["-h"; "-help"; "--help"], "Print all options",
   Arg.Unit print_usage);
]
