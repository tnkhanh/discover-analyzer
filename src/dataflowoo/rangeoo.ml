...
module ID = IntervalData

type t = ID.t

class range_transfer = object(self)
  (* open IntervalDomain *)

  inherit [t] DF.forward_data_transfer DfaRange
  ...
  method pr_data_checksum = pr_data

  (* comparison*)

  method equal_data (a: t) (b: t) : bool =
    MP.equal equal_interval a b

  method lequal_data (a: t)  (b: t) : bool =
  ...
  method subst_data ?(sstv: substv = []) ?(sstve: substve = [])
        ?(sste: subste = []) (d: t) : t =
  ...
  method merge_data ?(widen=false) (a: t) (b: t) : t =
  ...
  method join_data (a: t) (b: t) : t =
    merge_data a b

  method get_interval (e: expr) (d: t) : ID.interval =
    match e with
  ...
  method replace_interval (e: expr) (i: interval) (d: t) : t =
    let nd = match MP.find d e with
      | None -> d
      | Some a -> MP.remove d e in
    MP.add_exn ~key:e ~data:i nd

  method clean_irrelevant_info_from_data penv func (d: t) : t =
    MP.filteri ~f:(fun ~key:e ~data:i ->
      let vs = collect_llvalue_of_expr e in
      not (List.exists ~f:is_local_llvalue vs)) d

  method clean_info_of_vars (input: t) (vs: llvalues) : t =
    (* TODO: implement later *)
    input

  method extract_data_from_predicate ~(widen: bool) (p: predicate) (d: t) : t =
    match p with
    | PBool _ -> MP.empty
    | PIcmp (cmp, lhs, rhs) ->
  ...
  method is_data_satisfied_predicate (d: t) (p: predicate) : bool =
  ...
  method refine_data_by_predicate ?(widen=false) (d: t) (p: predicate) : t =
  ...
  method need_widening func : bool =
    true

  method refine_widening func instr widen : bool =
  ...
  method prepare_callee_input penv instr func args (input: t) : t =
    input

  method compute_callee_output_exns penv instr callee args input fsum : t * exns =
    (input, [])

  method prepare_thrown_exception_data penv exn_ptr tinfo input : t =
    input

  method compute_catch_exception_data penv instr ptr input exn : t =
    input

  method analyze_global (g: global) (input: t) : t =
    (* TODO: default behavior, implement later if necessary *)
    input

  method analyze_instr ?(widen=false) (penv: prog_env) fenv instr (input: t) : t =
  ...
  method check_buffer_overflow fenv (bof: BG.buffer_overflow) : ternary =
  ...
  method check_integer_overflow fenv (iof: BG.integer_overflow) : ternary =
  ...
  method check_integer_underflow fenv (iuf: BG.integer_underflow) : ternary =
  ...
  method check_bug (fenv: func_env) (bug: BG.bug) : ternary =
  ...
  method count_assertions (prog: program) : int =
  ...
  method check_range_lower_bound fenv instr (v: llvalue) (lb: int64) : bool =
  ...
  method check_range_upper_bound fenv instr (v: llvalue) (ub: int64) : bool =
    match get_instr_output fenv instr with
    | None -> false
    | Some data ->
      match get_interval (expr_of_llvalue v) data with
  ...
  method check_range_full fenv instr (v: llvalue) (lb: int64) (ub: int64) : bool =
    (check_range_lower_bound fenv instr v lb) &&
    (check_range_upper_bound fenv instr v ub)

  method check_assertion (fenvs: func_env list) (ast: AS.assertion) : bool option =
  ...
  method check_assertions (penv: prog_env) func : int =
